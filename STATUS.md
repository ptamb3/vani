...
**Last updated:** 2026-06-01 (closure #497 — **`hash_combine_3(a: u64, b: u64, c: u64) -> u64`** 3-arg hash combiner. Defined as `hash_combine(hash_combine(a, b), c)` — left-fold the existing boost-style FNV-tuned mixer. Bounded, atomic stepping stone toward Arc 4 tuple-keyed `HashMap<Tuple<...>, V>`: per-element hashes chain through this combiner. Both backends byte-identical: with `hash_i64(1/2/3)` as inputs the result interprets as i64 `-415690348689364798`, equal to the manual two-step `hash_combine(hash_combine(a, b), c)`. C: inline composition of `intent_hash_combine` calls. LLVM: two sequential `@intent_hash_combine` calls. 1 new lib test. 1512 lib + 54 parity green.)
