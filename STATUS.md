...
**Last updated:** 2026-05-31 (closure #421 — **`i64_count_digits(n) -> i64`** decimal digit count. Returns the number of decimal digits in `|n|`; `count_digits(0) = 1` (the digit '0'); negatives count the digits of `|n|`, sign is not a digit. Handles the `INT64_MIN` edge correctly via unsigned cast (C: `(uint64_t)(-(n+1))+1`; LLVM: select + udiv to step through the unsigned magnitude). C: inline GNU statement-expression with while loop. LLVM: emits a shared `@intent_i64_count_digits(i64) -> i64` helper alongside `@intent_i64_isqrt`, with the same alloca-per-state + br-back-to-head loop shape. Both backends byte-identical: `count_digits(0) = 1`, `count_digits(5) = 1`, `count_digits(10) = 2`, `count_digits(99) = 2`, `count_digits(100) = 3`, `count_digits(-7) = 1`, `count_digits(-12345) = 5`, `count_digits(MAX) = 19`, `count_digits(MIN) = 19` (MIN's magnitude is 9223372036854775808 = 19 digits). 2 new lib tests (typecheck + helper define + call emission). 1430 lib + 54 parity green. Closure #420 (float trunc/frac) shipped immediately before.)
