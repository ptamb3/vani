...
**Last updated:** 2026-05-31 (closure #403 — **IEEE-754 bit-level access**: `f64_to_bits(x: f64) -> i64` and `f64_from_bits(b: i64) -> f64`. Both are zero-cost reinterprets of the underlying bits. C: routes through `memcpy(&dst, &src, sizeof)` — the strict-aliasing-safe spelling that gcc/clang both optimize to a no-op move at `-O1+`. LLVM: routes through `bitcast double {} to i64` / `bitcast i64 {} to double` — zero-cost in IR. The checker special-cases both before the F64 coerce loop so the return types stay accurate (i64 for to_bits, f64 for from_bits). Both backends byte-identical: `1.0 → 0x3FF0000000000000`, `-1.0 → 0xBFF0000000000000`, `0.0 → 0`; round-trip via from_bits(to_bits(x)) preserves bits. 2 new lib tests pin typecheck + LLVM bitcast emission. 1401 lib + 54 parity green. Closure #402 (bswap + rotate) shipped immediately before.)
