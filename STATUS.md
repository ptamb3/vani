...
**Last updated:** 2026-05-29 (closure #336 — **CSR adjacency cache on the existing `Graph` type**: BFS and DFS now run in O(V+E) instead of O(V·E). Three new struct fields (`adj_start: i32*` + `adj_csr_dst: i32*` + `adj_csr_weight: i64*`) are appended at indices 6/7/8 so existing LLVM GEP indices for fields 0..5 stay valid; struct byte_size grows from 48 to 72. A new `intent_graph_build_csr_if_needed` helper does a lazy two-pass build (count out-degrees per source, prefix-sum into adj_start, bucket-scatter edges into adj_csr_dst/weight). `add_edge` now invalidates the cache (frees + nulls the three CSR fields); the next traversal triggers a rebuild. `Drop` also frees the cache. BFS and DFS now iterate neighbors via `adj_start[u] .. adj_start[u+1]` instead of scanning all edges, dropping per-pop edge iteration from O(num_edges) to O(degree). The remaining algorithms (has_cycle, dijkstra, astar, prim, kruskal, topo_sort) still use the original per-edge scan in v1 — promoting them to CSR is queued as a follow-up. New `examples/graph_csr.vani` exercises a denser 8-node tree-plus-cross-edge graph + add_edge-invalidation roundtrip + isolated/single-node corner cases; both backends produce identical output. 1 new lib test pins the new helper names + 9-field LLVM type. Closures #334 (A*) + #335 (topo_sort) shipped immediately before.)
