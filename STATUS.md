...
**Last updated:** 2026-05-31 (closure #411 — **`i64_min / i64_max / i64_clamp`** + **`f64_min / f64_max / f64_clamp`** scalar binary min / max / clamp. `i64_min(a, b)` and `i64_max(a, b)` return the smaller / larger of two signed 64-bit ints. `i64_clamp(x, lo, hi)` returns `max(lo, min(hi, x))` — clamps `x` into the closed interval `[lo, hi]`. The float versions are analogous but use the IEEE-754 "NaN-as-missing" intrinsics on the LLVM side. C: i64 ops are inline statement-expressions with C ternary on signed compare; f64 ops emit `fmin` / `fmax` from `<math.h>` (already linked via `-lm`). LLVM: i64 ops emit `icmp slt/sgt` + `select` (no intrinsic — codegen is small enough inline); f64 ops emit `@llvm.minnum.f64` / `@llvm.maxnum.f64`; `i64_clamp` / `f64_clamp` chain two select instructions. Both backends byte-identical: `i64_min(5, 3) = 3`, `i64_max(5, 3) = 5`, `i64_min(-10, -20) = -20`, `i64_clamp(5, 0, 10) = 5`, `i64_clamp(-5, 0, 10) = 0`, `i64_clamp(50, 0, 10) = 10`, `f64_min(1.5, 2.5) = 1.5`, `f64_clamp(-0.5, 0, 1) = 0`, `f64_clamp(1.5, 0, 1) = 1`. 2 new lib tests. 1411 lib + 54 parity green. Closure #410 (saturating arithmetic) shipped immediately before.)
