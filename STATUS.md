...
**Last updated:** 2026-06-01 (closures #505/#506 — **`vec_argmin(xs: ref Vec<i64>, def: i64) -> i64`** and **`vec_argmax(xs: ref Vec<i64>, def: i64) -> i64`** index-of-min/max reductions. Mirror of `vec_min/vec_max` (#322) but return the *index* of the extremum instead of the value. Empty Vec returns the supplied default (typical sentinel: `-1`). Implementation: single-pass tracking both the running best value and its index. C: `__argmin`/`__argmax` helpers added next to `__min`/`__max` in the vec-helper preamble. LLVM: parallel define blocks with `select` for branchless update of value+index. Both backends byte-identical: with `xs = [30, 10, 50, 20]`, `argmin = 1`, `argmax = 2`; empty Vec → `-1`. 1 new lib test. 1517 lib + 54 parity green.)
