...
**Last updated:** 2026-05-29 (closure #352 — **`btreeset_min` / `btreeset_max` + `btreemap_min_key` / `btreemap_max_key`**: O(1) ends-access on the sorted-Vec backings, parallel to `skiplist_min` / `skiplist_max` (#341). The BTree containers store keys in ascending order so `keys[0]` is the smallest and `keys[len-1]` is the largest — no scan needed, no rebalance involved. All four return `Option<i64>` (None on empty); the BTreeMap variants return the key only (paired value reads via `btreemap_get(m, min_key)` after unwrap). Method sugar: `s.min()` / `s.max()` on BTreeSet, `m.min_key()` / `m.max_key()` on BTreeMap. Helpers gated on `Option__i64` registry being present (auto-mono walker registers Option<i64> when any of the four names is called). Extended `examples/btreeset.vani` + `examples/btreemap.vani` exercise the helpers via method sugar. Both backends byte-identical. 4 new lib tests pin typecheck + helper-name emission. 1280 lib + 54 parity green across 90 examples. Closure #351 (SipHash-2-4) shipped immediately before. Note: closure #352 was re-prioritized over the queued Hash/Ord interface (now #353+) because Hash/Ord container monomorphization is genuinely multi-session — landing this quick O(1) API completion first.)
