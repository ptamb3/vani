...
**Last updated:** 2026-05-29 (closure #350 — **`str_split(s: Str, delim: Str) -> Vec<OwnedStr>`**: third and final heap-allocating string builtin (joins `str_trim` #348 and `str_replace` #349 — the trio is now complete). Returns a fresh `Vec<OwnedStr>` whose elements are non-overlapping spans of `s` between occurrences of `delim`. Each element is a freshly-malloc'd `char*`; the Vec's per-element Drop chain frees every span on scope exit. Single-pass scan via libc `strstr`, with inline push onto the output Vec (no per-type-helper dependency) — capacity-doubling growth starting at 4. Edge cases: empty `delim` returns a single-element vec containing `dup(s)` (splitting on `""` is otherwise ill-defined); empty `s` returns a single-element vec with an empty OwnedStr; consecutive delims yield empty middle elements; leading/trailing delims yield empty bracket elements; no-match returns a single-element vec with `dup(s)`. Iteration uses `clone_at(ref xs, i)` since OwnedStr is non-Copy (would alias the owner's slot and double-free if indexed by value). Helper emission is gated on actual usage via a new `program_uses_str_split` walker — the helper's signature references `%intent_vec_i8p` (LLVM) / `intent_vec_owned_str` (C), which are themselves element-walker-gated; emitting unused str_split would leave a dangling type reference. Both backends emit the helper after the Vec typedef is in scope. Extended `examples/string_ops.vani` covers basic split, multi-char delim, consecutive delims, and no-match. Both backends byte-identical. 3 new lib tests pin typecheck (returns `Vec<OwnedStr>`), helper-name emission, and unused-gating. 1272 lib + 54 parity green across 90 examples. Closure #349 (`str_replace`) shipped immediately before.)
