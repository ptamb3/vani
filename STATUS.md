...
**Last updated:** 2026-05-31 (closure #417 — **`f64_is_normal(x) -> bool`** + **`f64_is_subnormal(x) -> bool`** + **`f64_sign_bit(x) -> bool`** IEEE-754 classification predicates. Together with the previously-shipped `f64_is_nan / f64_is_inf / f64_is_finite` these complete the float-introspection predicate surface. `f64_is_normal(x)` is true iff x is a normal (non-zero, non-subnormal, finite, non-NaN) double. `f64_is_subnormal(x)` is true iff x is a positive or negative denormalized number. `f64_sign_bit(x)` returns the sign bit (true for negative including -0.0, false for positive including +0.0). C: routed through `isnormal()`, `fpclassify() == FP_SUBNORMAL`, and `signbit()` macros from `<math.h>`. LLVM: emitted as inline bitcast + bit-manipulation — `bitcast double to i64`, then `lshr 52` to extract the 11-bit exponent (mask `2047`), `lshr 63` for the sign bit, and `and i64 ..., 4503599627370495` (2^52 - 1) for the mantissa. `is_normal` = `exp != 0 && exp != 2047`; `is_subnormal` = `exp == 0 && mantissa != 0`; `sign_bit` = `bit 63 != 0`. Both backends byte-identical: `is_normal(1.5) = true`, `is_normal(0) = false`, `is_normal(inf) = false`, `is_normal(nan) = false`, `is_normal(min_subnormal) = false`; `is_subnormal(min_subnormal) = true`; `sign_bit(1.5) = false`, `sign_bit(-3.0) = true`. 2 new lib tests (typecheck + LLVM bit-manipulation shape). 1423 lib + 54 parity green. Closure #416 (IEEE math primitives) shipped immediately before.)
