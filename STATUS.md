...
**Last updated:** 2026-05-30 (closure #391 — **`option_and_then(o, f) -> Option<i64>`** flatmap with method sugar `o.and_then(f)`. `f: fn(i64) -> Option<i64>` is invoked on the Some-payload, and its result (which is itself an Option) is returned directly — no double-wrapping. None passes through. C: inline statement-expression. LLVM: 2-way branch (some calls f, none builds an empty Option) with phi at the merge. Method sugar registered in the Option-method dispatch table alongside `.map / .filter / .or / .unwrap_or / .is_some / .is_none`. Both backends byte-identical. 2 new lib tests pin typecheck + wrong-return-type rejection. 1383 lib + 54 parity green. Closure #390 (str_chars + str_reverse) shipped immediately before.)
