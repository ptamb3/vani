...
**Last updated:** 2026-05-31 (closure #409 — **`i64_is_power_of_2(n) -> bool + i64_next_power_of_2(n) -> i64`** power-of-2 helpers. `is_power_of_2(n)` = `n > 0 && (n & (n-1)) == 0` — the classic single-bit-set check. `next_power_of_2(n)` returns 1 for n <= 1, else the smallest power of 2 >= n via `1 << (64 - clz(n - 1))` — preserves identity for n = 2^k. C: inline statement-expressions with `__builtin_clzll`. LLVM: explicit icmp/and/sub sequence for is_power_of_2; ctlz + shl + select for next_power_of_2. Both backends byte-identical: `is(8) = true`, `is(3) = false`, `is(0) = false`, `is(-4) = false`; `next(0) = 1`, `next(7) = 8`, `next(8) = 8`, `next(100) = 128`. 1 new lib test. 1407 lib + 54 parity green. Closure #408 (integer log2) shipped immediately before.)
