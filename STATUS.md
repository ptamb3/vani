...
**Last updated:** 2026-05-31 (closure #459 — **`i64_max_3(a, b, c) -> i64`** three-argument max. Companion to closure #458's `i64_min_3`. Both share a unified LLVM dispatch arm that picks `icmp slt` vs `icmp sgt` based on the builtin name, then chains two `select` instructions to fold three values. C: inline chained ternary with `>` instead of `<`. Both backends byte-identical: `max3(5, 3, 7) = 7`, `max3(7, 3, 5) = 7`, `max3(3, 5, 7) = 7`, `max3(-5, 0, 5) = 5`, `max3(0, 0, 0) = 0`, `max3(MAX, 0, MIN) = MAX`. 1 new lib test. 1483 lib + 54 parity green. Closure #458 (min_3) shipped immediately before.)
