...
**Last updated:** 2026-05-31 (closure #441 — **`str_byte_count(s, b) -> i64`** count occurrences of byte `b` in string `s`. Walks the string until the null terminator, incrementing a counter on each match. Useful for delimiter-counting (e.g., commas in CSV, newlines for line count) without going through the heavier `str_count_char` which takes a one-character `Str` and requires comparing bytes through a string pointer. C: inline GNU statement-expression with a `for` walk over the string. LLVM: emits `@intent_str_byte_count(i8*, i64) -> i64` helper alongside the existing `@intent_str_count_char`. The helper truncates the i64 byte to i8 once, then loops through the string comparing each loaded i8 to that target. Both backends byte-identical: in `"hello world"` (with byte values 'l' = 108, 'o' = 111, ' ' = 32), `count('l') = 3`, `count('o') = 2`, `count(' ') = 1`, `count('x') = 0`; in `"a,b,c,d,e"`, `count(',') = 4`; in `"aaaa"`, `count('a') = 4`; empty string yields 0 for any byte. 2 new lib tests. 1460 lib + 54 parity green. Closure #440 (sinc/safe_div) shipped immediately before.)
