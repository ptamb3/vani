...
**Last updated:** 2026-05-31 (closure #431 — **`i64_binomial(n, k) -> i64`** binomial coefficient C(n, k). Defensive defaults: returns 0 for `k < 0`, `n < 0`, or `k > n`; returns 1 for `k == 0` or `k == n`. Uses the symmetry `C(n, k) = C(n, n-k)` to minimize iterations. Iterative formula `r = r * (n - i + 1) / i` — exact at each step, but the intermediate product can overflow even when the final result fits. **Saturates to `INT64_MAX` on overflow** (detected via `__builtin_mul_overflow` in C, `@llvm.smul.with.overflow.i64` in LLVM) so overflow is visible to the caller rather than producing silently wrong results. C: inline GNU statement-expression. LLVM: emits a shared `@intent_i64_binomial(i64, i64) -> i64` helper. Both backends byte-identical: `C(5, 2) = 10`, `C(10, 5) = 252`, `C(20, 10) = 184756`, `C(0, 0) = 1`, `C(5, 0) = C(5, 5) = 1`, `C(5, 1) = C(5, 4) = 5`, `C(50, 25) = 126410606437752` (correct exact value), `C(-1, 2) = C(5, -1) = C(5, 10) = 0` (defensive), `C(66, 33) = 9223372036854775807` (intermediate product overflows, saturates to INT64_MAX). 2 new lib tests. 1448 lib + 54 parity green. Closure #430 (step/smoothstep) shipped immediately before.)
