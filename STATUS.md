...
**Last updated:** 2026-05-30 (closure #378 — **`vec_position(ref xs, pred) -> Option<i64>` + method sugar + closure-shorthand return-type inference**. New builtin: walks `xs` sequentially returning `Option.Some(i)` on the first element where `pred(x)` is true, or `Option.None` otherwise. C / LLVM emit a per-Vec helper alongside the existing filter / fold / map family, gated on `Option<i64>` being in the payload registry (the `Enum_Option__i64` struct must be in scope). Method sugar `xs.position(...)` registered alongside the other Vec methods. Bonus follow-up to #374: the anonymous-fn shorthand `|x| ...` now infers `Bool` return type when the body's top-level operator is a comparison (`==`/`!=`/`<`/`<=`/`>`/`>=`/`&&`/`||`/`!`) or a bool literal — fixes a paper cut where `|x| x > 5` previously produced `fn(i64) -> i64` and bounced off `vec_filter` / `vec_position` predicate-type checks. The walker registering `Option<i64>` for the auto-mono pass now lists `vec_position`. 3 new lib tests pin free-function path + method sugar + bool-return shorthand inference. 1353 lib + 54 parity green. Closure #377 (option_map) shipped immediately before.)
