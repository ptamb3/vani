...
**Last updated:** 2026-05-30 (closure #365 — **`str_index_of(haystack: Str, needle: Str) -> Option<i64>`**. Pairs with the existing `str_contains` (which returns bool) to give the byte offset of `needle` inside `haystack`, or `Option.None` if missing. C: inline statement-expression — `strstr` + null-check + `Enum_Option__i64` construction with the byte-difference payload. LLVM: explicit branch — `call i8* @strstr`, `icmp eq … null`, branch to `some`/`none` blocks, in the `some` block `ptrtoint` both pointers + `sub i64` to compute the offset, `insertvalue` into `%Enum_Option__i64`, then `phi` at the merge. Auto-monomorphization walker extended so a bare `str_index_of(...)` call forces the `Option__i64` decl even if no other Option<i64>-producing builtin is in scope. Both backends byte-identical: needle in middle → offset 7; needle absent → None; needle at start → offset 0. 3 new lib tests pin typecheck + strstr-emit + Enum_Option__i64 construction + None-path. 1317 lib + 54 parity green. Closure #364 (f64 classification) shipped immediately before.)
