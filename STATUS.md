...
**Last updated:** 2026-05-31 (closure #406 — **`f64_lerp(a, b, t) + f64_clamp01(x)`** graphics/numerics ergonomics. `f64_lerp` is the standard `a + (b - a) * t` linear interpolation; `f64_clamp01` is `max(0, min(1, x))`. C: inline statement-expressions. LLVM: explicit fsub/fmul/fadd for lerp; nested fcmp + select for clamp01. Falls through the existing f64-coerce loop in `check_math_builtin` with `f64_lerp` getting `want_args = 3`. Both backends byte-identical: `f64_lerp(5, -5, 0.25) = 2.5`, `f64_clamp01(1.5) = 1.0`, `f64_clamp01(-0.5) = 0.0`. 1 new lib test. 1404 lib + 54 parity green. Closure #405 (floor division) shipped immediately before.)
