...
**Last updated:** 2026-05-30 (closure #397 — **`vec_zip_with(ref xs, ref ys, f: fn(i64, i64) -> i64) -> Vec<i64>`** with method sugar `xs.zip_with(ref ys, f)`. Pairwise applies `f` to corresponding elements of `xs` and `ys`, truncating to the shorter Vec. Sidesteps the tuple-element-Vec dependency that full `vec_zip(xs, ys) -> Vec<(i64, i64)>` would need. C: dedicated per-Vec helper with a typedef'd `__zip_fn` pointer. LLVM: parallel define with phi-driven loop, `min(xlen, ylen)` via `icmp ult + select`, and per-iteration paired getelementptr + indirect call. Method sugar `.zip_with(ref ys, f)` registered alongside the other Vec method-dispatch entries. Both backends byte-identical: `[1,2,3,4,5].zip_with([10,20,30], +) = [11,22,33]` (truncated to len-3). 2 new lib tests pin typecheck + unequal-length truncation. 1391 lib + 54 parity green. Closure #396 (vec_replace_all) shipped immediately before.)
