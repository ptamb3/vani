...
**Last updated:** 2026-05-31 (closure #401 — **bit-manipulation primitives**: `i64_count_set_bits(x)` (popcount), `i64_leading_zeros(x)` (clz), `i64_trailing_zeros(x)` (ctz). All take i64 and return i64. C: routes through GCC/Clang `__builtin_popcountll / clzll / ctzll`, with explicit zero-input guards for clz/ctz (which are undefined at 0 per the ABI; we define them as 64 — the bit width — for ergonomic consistency). LLVM: routes through `@llvm.ctpop.i64 / @llvm.ctlz.i64 / @llvm.cttz.i64` intrinsics declared in the preamble. The `is_zero_poison` flag is set to `i1 false` so ctlz(0) = cttz(0) = 64 in the IR — matches the C path. Both backends byte-identical: `popcount(7)=3`, `clz(1)=63`, `ctz(8)=3`, `popcount(-1)=64`. 2 new lib tests pin typecheck + LLVM intrinsic declare/call emission. 1397 lib + 54 parity green. Closure #400 (ASCII byte-class predicates) shipped immediately before.)
