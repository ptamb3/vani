...
**Last updated:** 2026-05-30 (closure #389 — **`vec_take_while(ref xs, pred) -> Vec<i64>` + `vec_drop_while(ref xs, pred) -> Vec<i64>`**. Predicate-based prefix slicing. `take_while` returns a fresh Vec containing the longest prefix where `pred(x)` is true; `drop_while` returns the suffix after that prefix. Same `fn(i64) -> bool` predicate shape as the rest of the family. C: dedicated per-Vec helpers emitted alongside `take` / `drop` / `count_if`. LLVM: parallel define pair generated from a loop over `(helper_op, scan_keep_pred)` tuples, sharing the same scan-then-memcpy shape. Method sugar `.take_while(p)` / `.drop_while(p)` registered alongside the other Vec methods. Both backends byte-identical: `[1,3,5,4,7,9].take_while(odd) = [1,3,5]`, `drop_while(odd) = [4,7,9]`. 2 new lib tests pin typecheck + method sugar. 1379 lib + 54 parity green. Closures #387 + #388 (vec_swap + vec_remove_at) shipped immediately before.)
