...
**Last updated:** 2026-05-31 (closure #412 — **`i64_isqrt(n) -> i64`** integer square root. Returns `floor(sqrt(n))` for `n ≥ 0`; returns `0` for negative input (defensive default — matches the convention of other i64 helpers that don't trap on out-of-domain input). Algorithm: Heron / Newton's method on integers — `x = n; y = (x + n/x) / 2; while (y < x) { x = y; y = (x + n/x) / 2; } return x`. Converges in O(log log n) iterations. C: inline GNU statement-expression with the iteration directly written out. LLVM: emits a shared `@intent_i64_isqrt(i64) -> i64` helper alongside `@intent_i64_gcd / lcm / pow`, with the same alloca-per-state + br-back-to-head loop shape used by `@intent_i64_pow`. Both backends byte-identical: `isqrt(0) = 0`, `isqrt(1) = 1`, `isqrt(4) = 2`, `isqrt(8) = 2`, `isqrt(9) = 3`, `isqrt(15) = 3`, `isqrt(16) = 4`, `isqrt(100) = 10`, `isqrt(101) = 10`, `isqrt(1000000) = 1000`, `isqrt(-5) = 0`, `isqrt(10^12) = 1000000`. 2 new lib tests (typecheck + helper define + call emission). 1413 lib + 54 parity green. Closure #411 (scalar min/max/clamp) shipped immediately before.)
