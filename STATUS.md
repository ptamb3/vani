...
**Last updated:** 2026-05-29 (closure #337 — **promote remaining graph algorithms to CSR**: `has_cycle`, `topo_sort`, `dijkstra`, and `astar` now iterate neighbors via the CSR adjacency cache from closure #336 instead of scanning all edges per visited vertex. Each helper calls `intent_graph_build_csr_if_needed` once at entry and then walks `adj_start[u]..adj_start[u+1]` for each pop, dropping per-pop edge iteration from O(num_edges) to O(degree). Overall costs: BFS/DFS/has_cycle/topo_sort all O(V+E); Dijkstra/A* still O(V²) outer-loop but O(V+E) total relaxations instead of O(V·E). MST Kruskal stays per-edge by design (it sorts edges, doesn't traverse adjacency). MST Prim stays per-edge for now because it uses an undirected interpretation that would need a reverse CSR; queued for a follow-up. New `intent_graph_build_csr_if_needed` calls are inserted at each helper's entry on both backends, so the cache is built lazily on first use and reused across subsequent traversals until `add_edge` invalidates it. No new struct fields, no API changes. Existing `examples/graph.vani`, `graph_algo.vani`, `graph_algo2.vani`, and `graph_csr.vani` all produce byte-identical output across both backends. 1244 lib tests + parity green. Closure #336 (CSR cache, BFS+DFS promotion) shipped immediately before.)
