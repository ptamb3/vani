...
**Last updated:** 2026-05-29 (closure #331 — **Level 4 #7** `SkipList<T>` probabilistic ordered set on a node arena. MAX_LEVEL fixed at 8 (v1); per-node forward[] indices are a flat `i32*` array of capacity × 8 entries with `-1` meaning "no successor at this level". Geometric level distribution driven by an LCG seed stored in the struct (deterministic across runs for reproducibility). 6 builtins: `skiplist_new()`, `skiplist_insert(mut ref, x) -> bool` (true on new; false on duplicate), `skiplist_contains(ref, x) -> bool`, `skiplist_len(ref) -> i64`, `skiplist_min(ref) -> Option<i64>` (O(1)), `skiplist_max(ref) -> Option<i64>` (O(n) level-0 walk in v1 — tail tracker queued). Method sugar: `sl.insert(x)` / `.contains(x)` / `.len()` / `.min()` / `.max()`. v1 i64 keys only. Scope-exit Drop frees the three arrays. New `examples/skiplist.vani` insert/contains demo + 6 new lib tests. Closures #326 (BinaryHeap<T>), #327 (Bloom), #328 (Bst<T>), #329 (Graph), and #330 (Trie) shipped immediately before. Roadmap: **Level 4 is now 7/9 complete** — the data-structures roadmap is essentially done; remaining queued items are extensions (AVL/RB rotations on Bst arena, CSR adjacency + A*/topo/Kruskal/Prim on Graph, radix-compressed Trie, SkipList remove + tail tracker).)
**Test totals:** 1233 lib + 54 end-to-end + 11 vtables-phase3 + 2 user-drop-by-ref + 1 ssa-examples tests passing; the cross-backend parity runner covers all 86 examples under `examples/`. (Win32 LLVM dispatch adds 4 host-gated tests that fire on Windows hosts only — futex/WaitOnAddress, CreateThread for tasks, plus the CreateThread fan-out parallel-for tests in tree-LLVM and SSA-LLVM.)
