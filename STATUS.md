...
**Last updated:** 2026-05-31 (closure #457 — **`str_count_ascii_whitespace(s) -> i64`** count ASCII whitespace bytes — space (32) plus the control range `9..13` (HT, LF, VT, FF, CR). Completes the granular ASCII-class counter family begun at #454 (digits / alpha / alphanumeric / whitespace). C: inline statement-expression — `b == 32 || (b >= 9 && b <= 13)`. LLVM: emits `@intent_str_count_ascii_whitespace(i8*) -> i64` helper — `icmp eq 32` OR `(icmp uge 9 + icmp ule 13)`, joined with `or i1`. Both backends byte-identical: `count('Hello, World!') = 1` (single space), `count('   ') = 3`, `count('') = 0`, `count('no_whitespace') = 0`, `count('a b c\td\ne') = 4` (two spaces + tab + LF), `count('   trim_me   ') = 6` (3 leading + 3 trailing). 1 new lib test. 1481 lib + 54 parity green. Closure #456 (alphanumeric-count) shipped immediately before.)
