...
**Last updated:** 2026-06-01 (closures #476-#479 — **`i64_set_bit / i64_clear_bit / i64_toggle_bit / i64_test_bit`** single-bit operations on i64. `set_bit(n, i) = n | (1 << i)`, `clear_bit(n, i) = n & ~(1 << i)`, `toggle_bit(n, i) = n ^ (1 << i)`, `test_bit(n, i) -> bool = (n >> i) & 1 != 0`. All four shipped in one batch (similar shape). LLVM: inline IR with `shl 1, i` for mask, then `or/and/xor` or `lshr+and+icmp` for test. Both backends byte-identical: `set(5, 1) = 7`, `clear(7, 1) = 5`, `toggle(5, 1) = 7`, `test(5, 0/1/2) = true/false/true` (5 = 101 binary). 1 new lib test covering all four. 1500 lib + 54 parity green. **Tier B at 12/13, mod_inverse (#475) shipped just before.**)
