...
**Last updated:** 2026-05-31 (closure #432 — **`f64_relu(x) -> f64`** + **`f64_leaky_relu(x, alpha) -> f64`** + **`f64_softplus(x) -> f64`** ML activation primitives. `relu(x) = max(0, x)` — Rectified Linear Unit, classic deep-learning activation. `leaky_relu(x, alpha) = x if x >= 0 else alpha*x` — avoids the "dying ReLU" problem by giving a small slope on the negative side. `softplus(x) = log(1 + exp(x))` — smooth approximation of relu, differentiable everywhere. C: inline statement-expressions using libm `log`/`exp`. LLVM: `relu` uses `fcmp ogt` + `select`; `leaky_relu` uses `fcmp oge` + `fmul` + `select`; `softplus` uses `call @exp` + `fadd` + `call @log`. Both backends byte-identical: `relu(1.5) = 1.5`, `relu(-1.5) = 0`, `relu(0) = 0`; `leaky_relu(1.5, 0.01) = 1.5`, `leaky_relu(-1.5, 0.01) = -0.015`; `softplus(0) = log(2) = 0.693147`, `softplus(1) = log(1+e) = 1.31326`, `softplus(-1) = 0.313262`, `softplus(10) ≈ 10` (e^10 dominates). 1 new lib test. 1449 lib + 54 parity green. Closure #431 (binomial) shipped immediately before.)
