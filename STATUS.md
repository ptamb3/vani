...
**Last updated:** 2026-06-01 (closures #507/#508/#509 — **`vec_count_value(xs: ref Vec<i64>, v: i64) -> i64`**, **`vec_index_of_value(xs: ref Vec<i64>, v: i64) -> i64`**, **`vec_last_index_of_value(xs: ref Vec<i64>, v: i64) -> i64`**. Value-search reductions on `Vec<i64>` — count occurrences, first index, last index of a specific i64. Non-predicate forms (take a target value instead of `fn(i64)->bool`), so callers don't need a helper fn. `index_of_value` / `last_index_of_value` return `-1` when absent; `count_value` returns `0`. Both backends byte-identical: with `xs = [10,20,30,20,10,20]` and `v=20`, count=3 / first=1 / last=5; miss returns `-1`; empty Vec returns the zero/sentinel value cleanly. C: 3 new `__count_value`/`__index_of_value`/`__last_index_of_value` helpers in the vec preamble. LLVM: 3 parallel define blocks, `last_index_of_value` decrement-loop guarded by an `icmp eq 0` to avoid u64 underflow. 1 new lib test. 1518 lib + 54 parity green.)
