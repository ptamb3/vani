...
**Last updated:** 2026-06-01 (closures #516-#519 — **`vec_all_equal`**, **`vec_is_sorted_asc`**, **`vec_is_sorted_desc`**, **`vec_is_palindrome`** (each `xs: ref Vec<i64> -> bool`). Bool-returning Vec<i64> predicates: all elements equal / non-decreasing / non-increasing / sequence equals its reverse. Empty Vec or 1-element Vec is vacuously true for each. The first three share a fail-fast pairwise compare loop (first-vs-each for `all_equal`, prev-vs-cur for sorted_asc/desc); LLVM body is emitted from one shared loop over `(op, cmp_pred, prefix, ref_kind)` quadruples. Palindrome uses a two-pointer walk. Both backends byte-identical across positive, negative, and empty cases. 1 new lib test. 1521 lib + 54 parity green.)
