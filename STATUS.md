...
**Last updated:** 2026-05-31 (closure #428 — **`i64_fibonacci(n) -> i64`** saturating Fibonacci. Returns `F(n)` where `F(0) = 0, F(1) = 1, F(n) = F(n-1) + F(n-2)`. Saturates to `INT64_MAX` for `n > 92` (since `F(92) = 7540113804746346429` is the largest Fibonacci that fits in i64, and `F(93)` would overflow). Returns 0 for negative n (defensive). Iterative two-variable algorithm — O(n) time, O(1) extra space. C: inline GNU statement-expression with the iteration. LLVM: emits a shared `@intent_i64_fibonacci(i64) -> i64` helper alongside `@intent_i64_factorial`. Both backends byte-identical: `F(0) = 0`, `F(1) = 1`, `F(2) = 1`, `F(3) = 2`, `F(10) = 55`, `F(20) = 6765`, `F(50) = 12586269025`, `F(92) = 7540113804746346429` (largest fit), `F(93) = INT64_MAX = 9223372036854775807` (saturates), `F(100) = INT64_MAX`, `F(-5) = 0` (defensive). 2 new lib tests. 1444 lib + 54 parity green. Closure #427 (factorial) shipped immediately before.)
