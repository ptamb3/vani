...
**Last updated:** 2026-06-01 (closure #496 — **`f64_remap(x, from_lo, from_hi, to_lo, to_hi) -> f64`** generalized 5-arg remap. Formula: `to_lo + (x - from_lo) · (to_hi - to_lo) / (from_hi - from_lo)`. No clamping — extrapolation works outside `[from_lo, from_hi]`. Required extending the math-route `want_args` infrastructure to support 5-arg primitives (first 5-arg builtin in the codebase). Both backends byte-identical: `remap(5, 0, 10, 0, 100) = 50`, `remap(0, 0, 10, 0, 100) = 0`, `remap(10, 0, 10, 0, 100) = 100`, `remap(0, -1, 1, 0, 255) = 127.5` (normalize from `[-1, 1]` to `[0, 255]`), `remap(15, 0, 10, 0, 100) = 150` (extrapolation past upper bound). **Tier D COMPLETE — 6 closures #491-#496.** 1 new lib test. 1511 lib + 54 parity green.)
