...
**Last updated:** 2026-05-31 (closure #438 — **`f64_smoothstep5(edge0, edge1, x) -> f64`** quintic (C^2-continuous) smoothstep. Polynomial `6t^5 - 15t^4 + 10t^3` has zero first AND second derivatives at both `t = 0` and `t = 1`, giving smoother (jerk-free) transitions than the cubic `smoothstep` (which is only C^1-continuous). Useful for high-quality animation curves, displacement maps, and antialiasing where the cubic form's discontinuous second derivative shows up as visible artifacts. C: inline GNU statement-expression with Horner-form polynomial evaluation. LLVM: inline IR — same clamp logic as `smoothstep` (via two `select` instructions), then `t*t = t^2`, `t^2 * t = t^3`, then the inner polynomial via `fmul` + `fsub` + `fmul` + `fadd`, all combined into `t^3 * (t * (t * 6 - 15) + 10)`. Both backends byte-identical: `smoothstep5(0, 1, 0.5) = 0.5` (symmetric around midpoint), `smoothstep5(0, 1, 0.25) = 0.103516` (smaller than the cubic's 0.15625 — quintic ramps up more slowly near edges), `smoothstep5(0, 1, 0.75) = 0.896484` (symmetric), `smoothstep5(0, 1, 0) = 0`, `smoothstep5(0, 1, 1.0) = 1`, out-of-range saturates, `smoothstep5(10, 20, 15) = 0.5` (proper normalization). 1 new lib test. 1456 lib + 54 parity green. Closure #437 (inv_sqrt/round_to) shipped immediately before.)
