...
**Last updated:** 2026-05-31 (closure #434 — **`f64_cbrt / f64_expm1 / f64_log1p`** libm numerical-stability helpers. `cbrt(x)` is the exact real cube root (handles negative x correctly — `cbrt(-8) = -2`, where `pow(-8, 1.0/3.0)` would return NaN). `expm1(x) = exp(x) - 1` and `log1p(x) = log(1 + x)` are the standard accurate-near-zero counterparts of `exp`/`log`. Both avoid catastrophic cancellation: `expm1(1e-10) = 1e-10` exactly (where `exp(1e-10) - 1` would lose ~10 decimal digits), and `log1p(1e-10) = 1e-10` exactly. C: routed through libm directly. LLVM: declares `@cbrt`, `@expm1`, `@log1p` and uses the same `f64_*` → libm-name prefix-strip dispatch as the special-functions family from #433. Both backends byte-identical: `cbrt(8) = 2`, `cbrt(27) = 3`, `cbrt(-8) = -2`, `cbrt(0) = 0`; `expm1(0) = 0`, `expm1(1) = e - 1 ≈ 1.71828`, `expm1(1e-10) = 1e-10`; `log1p(0) = 0`, `log1p(1) = log(2) ≈ 0.693147`, `log1p(1e-10) = 1e-10`. 1 new lib test. 1452 lib + 54 parity green. Closure #433 (special functions) shipped immediately before.)
