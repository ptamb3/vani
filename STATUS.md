...
**Last updated:** 2026-05-31 (closure #419 — **`i64_div_ceil(a, b) -> i64`** + **`i64_div_round(a, b) -> i64`** integer division companions to the existing `i64_div_floor`. `div_ceil` rounds the quotient toward +infinity: `div_ceil(7, 2) = 4`, `div_ceil(-7, 2) = -3`, `div_ceil(8, 2) = 4` (exact). `div_round` is round-half-away-from-zero: `div_round(5, 2) = 3` (5/2 = 2.5 rounds away from zero), `div_round(-5, 2) = -3`, `div_round(7, 3) = 2` (7/3 ≈ 2.33 rounds toward zero), `div_round(4, 2) = 2` (exact). C: inline statement-expressions — `div_ceil` does `sdiv + srem` then bumps quotient by 1 iff remainder ≠ 0 and signs match; `div_round` uses absolute values then negates if signs differ. LLVM: inline IR with `sdiv / srem / icmp / select` instructions mirroring the C logic. Both backends byte-identical across all sign-combinations and exact-divisor edge cases. 1 new lib test (typecheck + compile). 1426 lib + 54 parity green. Closure #418 (next-representable-double) shipped immediately before.)
