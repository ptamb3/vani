...
**Last updated:** 2026-06-01 (closures #499/#500 — **`hash_pair(a, b: i64) -> u64`** and **`hash_triple(a, b, c: i64) -> u64`** i64-tuple hashing sugar. Inline desugaring: `hash_pair(a, b) = hash_combine(hash_i64(a), hash_i64(b))`, triple analogous. These are the high-level surface that user code writes when keying a HashMap by an `(i64, i64)` or `(i64, i64, i64)` tuple — paves a flat path toward Arc 4. Both backends byte-identical: `hash_pair(10, 20) == 5316191137045915110_u64`, `hash_triple(10, 20, 30) == 4431214379588440655_u64`, each equal to the manual two/three-step expansion. C: inline composition. LLVM: three/five sequential `@intent_hash_i64` + `@intent_hash_combine` calls. 1 new lib test. 1514 lib + 54 parity green.)
