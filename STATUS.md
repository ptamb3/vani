...
**Last updated:** 2026-05-31 (closure #444 — **`i64_avg(a, b) -> i64`** overflow-safe floor average via the classic bit-twiddling identity: `(a & b) + ((a ^ b) >> 1)`. This formulation **never overflows** even when `a + b` would (e.g., `avg(INT64_MAX, INT64_MAX) = INT64_MAX` — the straightforward `(a+b)/2` would silently wrap). The arithmetic right shift makes the result floor toward `-∞` (matches Python's `(a + b) // 2`), so mixed-sign inputs land on the lower neighbor for ties (e.g., `avg(MIN, MAX) = -1`, not `0`, since the true average is `-0.5`). C: inline GNU statement-expression — `(a & b) + ((a ^ b) >> 1)` directly (relies on C's standard arithmetic shift on signed types — guaranteed on all major platforms even though C99 calls it "implementation-defined"). LLVM: inline IR with `and`, `xor`, `ashr 1`, `add`. Both backends byte-identical: `avg(4, 6) = 5`, `avg(5, 7) = 6`, `avg(10, 20) = 15`, `avg(-2, -4) = -3` (floor: -3, not -3.0 rounded toward zero); `avg(MAX, MAX) = MAX` (overflow-safe); `avg(MAX, MAX-1) = MAX-1` (floor of MAX - 0.5); `avg(MIN, MIN) = MIN`; `avg(MIN, MAX) = -1` (floor of -0.5); `avg(-1, -1) = -1`. 2 new lib tests. 1465 lib + 54 parity green. Closure #443 (last_index_of_byte) shipped immediately before.)
