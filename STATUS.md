...
**Last updated:** 2026-05-31 (closure #400 — **ASCII byte-class predicates**: `is_ascii_digit / is_ascii_alpha / is_ascii_alphanumeric / is_ascii_whitespace(b: i64) -> bool`. Locale-independent byte-range checks (matches the C-locale semantics of libc's ctype family without the locale dependency). `is_ascii_digit`: [48..=57]. `is_ascii_alpha`: [65..=90] ∪ [97..=122]. `is_ascii_alphanumeric`: union of those. `is_ascii_whitespace`: [9..=13] ∪ {32}. C: inline statement-expressions with combined range checks. LLVM: explicit icmp + and/or sequences (no libc dependency). All four return Type::Bool from check_math_builtin. Predicates compose with `vec_count_if` etc. via a user-fn wrapper (builtins aren't first-class fn ptrs, so a `fn dp(c: i64) -> bool { return is_ascii_digit(c); }` shim is needed for higher-order use). 2 new lib tests pin typecheck + composition. 1395 lib + 54 parity green. Closure #399 (vec_dot) shipped immediately before.)
