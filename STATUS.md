...
**Last updated:** 2026-05-31 (closure #450 — milestone — **`f64_safe_sqrt(x) -> f64`** + **`i64_safe_div(a, b, default) -> i64`** defensive math primitives. `safe_sqrt(x)` returns `sqrt(x)` for `x >= 0`, else `0` (instead of NaN). `i64_safe_div(a, b, default) = a/b` for `b != 0`, else `default`. Companions to the float `f64_safe_div` (#440). The i64 version must branch on `b == 0` since LLVM `sdiv` with zero divisor is undefined behavior (whereas `fdiv` on `0.0` returns `Inf`/`NaN` rather than UB). C: inline GNU statement-expressions with ternary guards. LLVM: `safe_sqrt` uses `fcmp olt 0` + `call @sqrt` + `select`; `i64_safe_div` uses `icmp eq 0` + labeled branches + `phi` to merge the default-vs-quotient result without performing `sdiv` in the divisor-is-zero path. Both backends byte-identical: `safe_sqrt(4) = 2`, `safe_sqrt(2) = √2 ≈ 1.41421`, `safe_sqrt(0) = 0`, `safe_sqrt(-5) = 0` (defensive — would have been NaN); `safe_div(10, 3, -1) = 3`, `safe_div(10, 0, -1) = -1`, `safe_div(-10, 3, 999) = -3`, `safe_div(-10, 0, 999) = 999`, `safe_div(0, 5, -1) = 0`, `safe_div(0, 0, 42) = 42`. 1 new lib test. 1473 lib + 54 parity green. Closure #449 (l1_norm/isqrt_ceil) shipped immediately before. **40 closures shipped in this session arc (#410–#450).**)
