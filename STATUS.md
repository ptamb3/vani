...
**Last updated:** 2026-05-31 (closure #425 — **`i64_is_prime(n) -> bool`** primality test via trial division with the 6k±1 optimization. Returns false for `n < 2`; true for `n in {2, 3}`; rejects multiples of 2 and 3 fast; then iterates `i = 5, 11, 17, ...` (each `+= 6`) and tests divisibility by both `i` and `i + 2` while `i*i ≤ n`. Skips composite candidates of the form `6k`, `6k+2`, `6k+3`, `6k+4` since they're all divisible by 2 or 3, so only `6k+1` and `6k-1` (= `6k+5`) need testing. C: inline GNU statement-expression with the loop. LLVM: emits a shared `@intent_i64_is_prime(i64) -> i1` helper alongside `@intent_i64_pow_mod`. Both backends byte-identical: `is_prime(0/1) = false`, `is_prime(2/3/5/7/11/13/97/101/997) = true`, `is_prime(4/9/25/49/100/1024) = false`, `is_prime(7919) = true` (1000th prime), `is_prime(-7) = false` (defensive — negatives not prime). 2 new lib tests. 1438 lib + 54 parity green. Closure #424 (pow_mod) shipped immediately before.)
