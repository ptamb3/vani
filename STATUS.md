...
**Last updated:** 2026-05-31 (closure #437 — **`f64_inv_sqrt(x) -> f64`** + **`f64_round_to(x: f64, digits: i64) -> f64`** reciprocal sqrt and decimal-place rounding. `inv_sqrt(x) = 1 / sqrt(x)` — common in graphics for vector normalization. `round_to(x, digits)` rounds `x` to `digits` decimal places (negative `digits` rounds to higher orders — e.g., `digits = -1` rounds to tens). Algorithm: `round(x * 10^digits) / 10^digits` using libm's `round` (half-away-from-zero, C99). `round_to` is mixed-typed `(f64, i64) -> f64` — checker has a dedicated special-case branch since the generic f64-coerce loop would coerce `digits` to f64. C: inline GNU statement-expressions using libm `pow` + `round` + `sqrt`. LLVM: declares `@round(double) -> double` (distinct from the existing i64-returning `@llround`); `inv_sqrt` emits `call @sqrt` + `fdiv 1.0`; `round_to` emits `sitofp` + `call @pow` + `fmul` + `call @round` + `fdiv`. Both backends byte-identical: `inv_sqrt(4) = 0.5`, `inv_sqrt(1) = 1`, `inv_sqrt(100) = 0.1`; `round_to(3.14159, 2) = 3.14`, `round_to(3.14159, 4) = 3.1416`, `round_to(-3.14159, 2) = -3.14`, `round_to(1234.5678, 0) = 1235`, `round_to(1234.5678, -1) = 1230` (round to tens). 1 new lib test. 1455 lib + 54 parity green. Closure #436 (byte starts/ends_with) shipped immediately before.)
