...
**Last updated:** 2026-05-30 (closure #367 — **f64 math constants: `f64_pi() / f64_e() / f64_inf() / f64_nan()`**. Four zero-arg builtins returning `f64`. C: literal decimal constants (`3.14159...`, `2.71828...`) plus `INFINITY` / `NAN` macros from `<math.h>`. LLVM: hex-float double literals — exact IEEE-754 bit patterns (`0x400921FB54442D18` for π, `0x4005BF0A8B145769` for e, `0x7FF0000000000000` for +Inf, `0x7FF8000000000000` for quiet NaN). The dispatch arm short-circuits before the per-arg coerce loop in `check_math_builtin`. SSA reject list extended. Cross-trip with #364's classifiers verified: `f64_is_inf(f64_inf()) == true`, `f64_is_nan(f64_nan()) == true`. 3 new lib tests pin typecheck + classifier composition + zero-arg arity. 1323 lib + 54 parity green. Closure #366 (substring) shipped immediately before.)
