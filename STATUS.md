...
**Last updated:** 2026-05-31 (closure #415 — **`f64_epsilon() / f64_min_positive() / f64_min_subnormal()`** IEEE-754 small-magnitude boundary constants. `f64_epsilon()` returns `DBL_EPSILON = 2^-52 ≈ 2.22e-16` — the smallest x for which `1.0 + x != 1.0`. `f64_min_positive()` returns `DBL_MIN = 2^-1022 ≈ 2.225e-308` — the smallest positive normal double. `f64_min_subnormal()` returns `2^-1074 ≈ 4.94e-324` — the smallest positive subnormal (denormal). C: emitted as decimal literals matching the canonical IEEE-754 values. LLVM: emitted as exact-bit-pattern hex doubles — `0x3CB0000000000000`, `0x0010000000000000`, `0x0000000000000001` — so no precision is lost in the IR. Together with the previously-shipped `f64_max_finite()` these complete the float-introspection constant surface. Both backends byte-identical: all three constants print to their canonical IEEE-754 decimal forms; `(1.0 + epsilon) - 1.0` evaluates to exactly `epsilon` (proving the round-trip). 2 new lib tests (typecheck + LLVM exact-hex emission). 1419 lib + 54 parity green. Closure #414 (inverse/hyperbolic trig) shipped immediately before.)
