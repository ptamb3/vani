...
**Last updated:** 2026-05-31 (closure #453 — **`f64_log_b(x, base) -> f64`** arbitrary-base logarithm via the algebraic identity `log_b(x) = log(x) / log(base)`. Generalizes the existing `log` (natural), `log2`, `log10` libm calls to any positive base ≠ 1. C: emits `(log((x)) / log((base)))` inline. LLVM: two `call @log` + `fdiv`. For invalid inputs (non-positive x or base, or base = 1) the libm `log` returns `NaN` or `Inf` — propagated rather than masked, so callers can detect via `f64_is_nan / f64_is_inf` if needed. Both backends byte-identical: `log_2(8) = 3` (2³ = 8), `log_10(100) = 2`, `log_3(81) = 4` (3⁴ = 81), `log_10(1) = 0`, `log_2(0.5) = -1` (2^-1 = 0.5), `log_e(e) = 1`. 1 new lib test. 1476 lib + 54 parity green. Closure #452 (harmonic/quadratic means) shipped immediately before.)
