...
**Last updated:** 2026-05-29 (closure #347 — **`hash_f64(x: f64) -> u64`**: completes the hash family (was `hash_i64` / `hash_str` / `hash_combine` from closure #301; the f64 variant was queued in the deferred list). FNV-1a over the IEEE-754 bit pattern: bitcast the `double` to a `u64`, then run the same 8-byte FNV-1a loop as `hash_i64`. Folds `-0.0` to `+0.0` before bitcast so the two values (which compare equal under `==`) share a hash. NaN payloads are NOT normalized — per IEEE-754, `NaN != NaN`, so callers needing NaN-aware membership should compare via `isnan()` rather than expect a single canonical hash. C uses `memcpy(&u, &x, 8)` (no aliasing UB); LLVM uses `bitcast double to i64` and tail-calls the existing `@intent_hash_i64`. Hash-helper gate (LLVM `program_uses_hash`) extended to detect the new builtin; C piggybacks on the loose `intent_hash_` substring check. Extended `examples/hash.vani` exercises determinism, distinct inputs producing distinct hashes, and the `-0.0`/`+0.0` fold. 3 new lib tests pin typecheck + helper-name emission in both backends. 1263 lib + 54 parity green across 90 examples. Closure #346 (BTreeSet/BTreeMap range queries) shipped immediately before.)
