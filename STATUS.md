...
**Last updated:** 2026-05-31 (closure #439 — **`f64_swish(x) -> f64`** + **`f64_logit(x) -> f64`** ML activation + its inverse. `swish(x) = x * sigmoid(x) = x / (1 + exp(-x))` is the "SiLU" activation popular in modern Transformer-family architectures (e.g., Gemma, LLaMA). `logit(x) = log(x / (1 - x))` is the inverse of `sigmoid` — maps `(0, 1)` back to `(-∞, +∞)`. C: inline statement-expressions using libm `exp` and `log`. LLVM: `swish` uses `fsub` + `call @exp` + `fadd` + `fdiv`; `logit` uses `fsub` + `fdiv` + `call @log`. Both backends byte-identical: `swish(0) = 0`, `swish(1) = 0.731059` (= sigmoid(1)), `swish(-1) = -0.268941` (= -sigmoid(-1)), `swish(10) = 9.99955` (= 10 * 0.99996); `logit(0.5) = 0`, `logit(0.25) = log(1/3) = -1.09861`, `logit(0.75) = log(3) = 1.09861`, `logit(0.9) = log(9) = 2.19722`. (Verify the round-trip: `sigmoid(logit(p)) = p` for p ∈ (0, 1).) 1 new lib test. 1457 lib + 54 parity green. Closure #438 (smoothstep5) shipped immediately before.)
