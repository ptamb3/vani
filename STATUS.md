...
**Last updated:** 2026-05-30 (closures #394 + #395 — **`str_strip_prefix(s, p) / str_strip_suffix(s, sfx) -> OwnedStr`** and **`str_count_char(s, ch) -> i64`**. All three are (Str, Str)-shaped. Strip helpers always return a fresh malloc'd copy: stripped iff prefix/suffix matches (and is non-empty); otherwise unchanged. Avoids introducing Option<OwnedStr> machinery — the empty-prefix case naturally returns the original. `str_count_char` counts occurrences of `ch[0]` in `s`; empty `ch` returns 0. C: dedicated helpers always-on (no Vec / Option deps). LLVM: parallel defines using strlen / strncmp / memcpy for the strip ops; explicit byte-loop for count_char. Method sugar `.strip_prefix / .strip_suffix / .count_char` registered alongside the rest of the str family. Both backends byte-identical: `"hello world".strip_prefix("hello ") = "world"`, `"file.txt".strip_suffix(".txt") = "file"`, `"banana".count_char("a") = 3`. 2 new lib tests. 1388 lib + 54 parity green. Closure #393 (sign helpers) shipped immediately before.)
