...
**Last updated:** 2026-05-29 (closure #348 — **`str_trim(s: Str) -> OwnedStr`**: first heap-allocating member of the string builtin family. Strips leading and trailing ASCII whitespace (space, `\t`, `\n`, `\v`, `\f`, `\r` — the standard `isspace()` set) and returns a fresh OwnedStr handle. The output is always heap-owned so the scope-exit Drop is well-defined; even no-op trims and empty inputs hand back a freshly-malloc'd buffer. Internal whitespace and UTF-8 byte sequences are preserved (Devanagari "vāṇī" round-trips intact). C uses a straightforward two-pointer scan + memcpy; LLVM uses a six-way OR of the whitespace bytes, scanning forward to find the first non-whitespace, then decrementing length-from-end. Both helpers are emitted unconditionally (matching the existing `intent_str_concat` convention) and tagged `INTENT_UNUSED` in C so unused programs strip the symbol at link time. Extended `examples/string_ops.vani` exercises mixed leading+trailing whitespace, internal-whitespace preservation, no-op trim, and all-whitespace input. Both backends byte-identical. 3 new lib tests pin typecheck (returns OwnedStr), concat-compatibility (`str_trim(a) + str_trim(b)`), and helper-name emission in both backends. 1266 lib + 54 parity green across 90 examples. Closure #347 (`hash_f64`) shipped immediately before.)
