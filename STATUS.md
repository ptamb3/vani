...
**Last updated:** 2026-05-29 (closure #355 — **`graph_clear` + `union_find_clear`**: completes the clear() suite across **all 12 containers** in the language (the 3-closure lump #353→#354→#355). `graph_clear` frees all edge storage + CSR / reverse-CSR caches (via the existing `invalidate_csr` helper) and resets `num_edges=0`, keeping `num_nodes` (the construction-time identity). `union_find_clear` resets to all-singletons state: `parent[i]=i`, `rank[i]=0` (via `memset`), `sets=n`, keeping `n`. Both return prior len (Graph: prior num_edges; UnionFind: prior sets count). Method sugar `g.clear()` and `uf.clear()`. Both backends byte-identical. 2 new lib tests pin typecheck + helper-name emission. 1287 lib + 54 parity green across 90 examples. Closures #353 (HashSet/HashMap/BTreeSet/BTreeMap clear) and #354 (Level 4 container clear suite) shipped immediately before.)
