...
**Last updated:** 2026-05-29 (closure #330 — **Level 4 #4** `Trie` prefix tree on a node arena (lowercase a-z keys v1). Backing struct `{ i32* children, u8* is_end, i64 num_nodes, i64 capacity, i64 num_words }` where `children` is a flat 26 × num_nodes index array (-1 = no child). 6 builtins: `trie_new()`, `trie_insert(mut ref, s: Str) -> bool` (true on new; false on duplicate or any non-a-z input char), `trie_contains(ref, s) -> bool` (exact match), `trie_starts_with(ref, prefix) -> bool`, `trie_len(ref) -> i64` (words), `trie_node_count(ref) -> i64`. Method sugar: `t.insert(s)` / `.contains(s)` / `.starts_with(p)` / `.len()` / `.node_count()`. Insert/lookup short-circuit on non-a-z so user code can ask without pre-filtering. Drop frees both arrays. New `examples/trie.vani` insert/contains/starts_with demo + 6 new lib tests. Closures #326 (BinaryHeap<T>), #327 (Bloom), #328 (Bst<T>), and #329 (Graph + BFS/DFS/Dijkstra) shipped immediately before. Roadmap: **Level 4 is now 6/9 complete** — next, skip list (#331), AVL/RB on Bst arena, CSR adjacency on Graph + extended algorithms (A* / topo / Kruskal / Prim).)
**Test totals:** 1227 lib + 54 end-to-end + 11 vtables-phase3 + 2 user-drop-by-ref + 1 ssa-examples tests passing; the cross-backend parity runner covers all 85 examples under `examples/`. (Win32 LLVM dispatch adds 4 host-gated tests that fire on Windows hosts only — futex/WaitOnAddress, CreateThread for tasks, plus the CreateThread fan-out parallel-for tests in tree-LLVM and SSA-LLVM.)
