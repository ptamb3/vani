...
**Last updated:** 2026-05-31 (closure #405 — **floor division: `i64_div_floor(a, b) / i64_mod_floor(a, b)`**. Python-style floor semantics: `i64_div_floor` rounds toward -infinity (not toward zero like C's `/`); `i64_mod_floor` produces a remainder whose sign matches the divisor (so `-7 mod_floor 3 = 2`, not `-1`). Computed as the standard sdiv/srem + adjust pattern: if remainder is non-zero AND `(a XOR b) < 0` (different signs), subtract 1 from quotient / add b to remainder. C: inline statement-expressions with the sign-XOR test. LLVM: explicit sdiv/srem + icmp/and/select sequence. Both backends byte-identical across signed permutations: `-7/2 = -4`, `7/-2 = -4`, `-7%3 = 2`, `7%-3 = -2`. 1 new lib test. 1403 lib + 54 parity green. Closure #404 (boundary constants) shipped immediately before.)
