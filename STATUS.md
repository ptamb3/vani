...
**Last updated:** 2026-05-31 (closure #413 — **`f64_hypot(a, b) -> f64`** + **`f64_to_radians(deg) -> f64`** + **`f64_to_degrees(rad) -> f64`** trig / geometry helpers. `f64_hypot(a, b)` is the IEEE-754 overflow-safe `sqrt(a*a + b*b)` — never overflows for in-range a, b even when `a*a` would. C: emits `hypot(a, b)` from `<math.h>` (libm already linked). LLVM: declares `@hypot(double, double)` and routes through it. `f64_to_radians(deg) = deg * (π/180)` and `f64_to_degrees(rad) = rad * (180/π)`. C uses inline multiplication with the constant written in full precision. LLVM uses `fmul double {x}, 0x3F91DF46A2529D39` for π/180 and `0x404CA5DC1A63C1F8` for 180/π — IEEE-754 hex literals matching the exact double bit pattern. Both backends byte-identical: `hypot(3, 4) = 5`, `hypot(5, 12) = 13`, `to_radians(180) ≈ π = 3.14159`, `to_radians(90) ≈ π/2 = 1.5708`, `to_degrees(π) = 180`. 2 new lib tests (typecheck + `@hypot` declare/call emission). 1415 lib + 54 parity green. Closure #412 (integer square root) shipped immediately before.)
