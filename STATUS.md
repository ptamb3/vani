...
**Last updated:** 2026-05-31 (closure #408 — **`i64_log2_floor(n) / i64_log2_ceil(n)`** integer log2. `log2_floor(n) = 63 - clz(n)` for n > 0; returns -1 sentinel for n <= 0 (log2 undefined). `log2_ceil(n) = log2_floor(n - 1) + 1` for n >= 2; 0 for n == 1; -1 for n <= 0. C: routes through `__builtin_clzll` with explicit zero-input guards. LLVM: routes through `@llvm.ctlz.i64(i64, i1 false)` + nested select for the zero/one boundary cases. Both backends byte-identical: `log2_floor(8) = 3`, `log2_floor(15) = 3`, `log2_floor(16) = 4`, `log2_ceil(7) = 3`, `log2_ceil(8) = 3`, `log2_ceil(9) = 4`, both return -1 for n=0. 1 new lib test. 1406 lib + 54 parity green. Closure #407 (Vec<i64> set ops) shipped immediately before.)
