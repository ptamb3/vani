...
**Last updated:** 2026-05-31 (closure #445 — **`f64_inv_lerp(a, b, x) -> f64`** + **`f64_chebyshev(x, y) -> f64`** inverse linear interpolation and Chebyshev (L^∞) distance. `inv_lerp(a, b, x) = (x - a) / (b - a)` returns the interpolation factor `t` such that `lerp(a, b, t) ≈ x`; perfect companion to the existing `f64_lerp` (closure #406). Useful for normalizing values to a `[0, 1]` range, building input → output remappings, and curve parameterization. `chebyshev(x, y) = max(|x|, |y|)` is the L^∞ norm for 2D coordinates — the king's-move distance on a chessboard. C: inline statement-expressions using fabs + ternary. LLVM: `inv_lerp` uses `fsub` + `fsub` + `fdiv`; `chebyshev` uses two `call @fabs` then `call @llvm.maxnum.f64` for IEEE-754 NaN-aware maximum. Both backends byte-identical: `inv_lerp(0, 1, 0) = 0`, `inv_lerp(0, 1, 0.5) = 0.5`, `inv_lerp(10, 20, 15) = 0.5` (midpoint), `inv_lerp(0, 10, 5) = 0.5`, `inv_lerp(5, 15, 0) = -0.5` (extrapolation works); `chebyshev(3, 4) = 4`, `chebyshev(-5, 2) = 5`, `chebyshev(0, 0) = 0`, `chebyshev(7, -7) = 7`. 1 new lib test. 1466 lib + 54 parity green. Closure #444 (overflow-safe i64_avg) shipped immediately before.)
