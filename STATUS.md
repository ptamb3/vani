...
**Last updated:** 2026-05-30 (closure #369 — **`str_to_upper(s: Str) / str_to_lower(s: Str) -> OwnedStr`**. ASCII case conversion (non-ASCII bytes pass through unchanged). C: dedicated helpers walk the source byte-by-byte applying a conditional ±32 delta when the byte is in the ASCII alpha range. LLVM: parallel `define i8* @intent_str_to_upper / @intent_str_to_lower(i8*)` with phi-driven loop blocks, `icmp uge/ule` for the range check, and `select` to pick between the shifted byte and the original. Both backends byte-identical: `str_to_upper("Hello, World! 123") = "HELLO, WORLD! 123"`. 3 new lib tests pin typecheck + helper-name emission + arity-1 requirement. 1329 lib + 54 parity green. Closure #368 (str_repeat) shipped immediately before.)
