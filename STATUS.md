...
**Last updated:** 2026-05-29 (closure #356 — **`Vec<i64>` utility lump: `vec_range` / `vec_repeat` / `vec_extend` / `vec_concat`**: four ergonomic constructors + combinators on the existing `intent_vec_int64_t` struct. `vec_range(lo, hi)` returns `[lo..hi)`; `vec_repeat(v, n)` returns n copies; `vec_extend(mut ref xs, ref ys)` appends ys to xs in-place (returns new len, with `realloc` growth doubling capacity); `vec_concat(ref xs, ref ys)` returns a fresh `xs ++ ys` with both inputs remaining valid. C uses `memcpy` for bulk copies; LLVM mirrors with bitcasted `@memcpy` calls. Helpers gated on the existing Vec<i64> walker (now extended to detect the new builtin names), emitted post-Vec-bundle in body so the `intent_vec_int64_t` typedef is in scope. Both backends byte-identical. 3 new lib tests pin typecheck, by-ref rejection on `vec_extend`'s first arg, and helper-name emission. 1290 lib + 54 parity green. Closure #355 (graph/UnionFind clear) shipped immediately before.)
