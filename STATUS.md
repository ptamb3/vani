...
**Last updated:** 2026-05-28 (closure #322 — Level 3 reduction combinators on Vec<i64>: `vec_sum` / `vec_product` (no extra args); `vec_min(ref xs, default)` / `vec_max(ref xs, default)` (default returned on empty); `vec_count(ref xs, p)` / `vec_any(ref xs, p)` / `vec_all(ref xs, p)` (take predicate; any/all short-circuit). Each reducible to vec_fold but ships as a dedicated single-pass builtin for readability + minimal-overhead codegen. Method sugar across all seven: `xs.sum()` / `xs.product()` / `xs.min(d)` / `xs.max(d)` / `xs.count(p)` / `xs.any(p)` / `xs.all(p)`. 5 new lib tests.)
