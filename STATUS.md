...
**Last updated:** 2026-05-31 (closure #435 — **`f64_exp2(x) -> f64`** + **`f64_exp10(x) -> f64`** base-2 and base-10 exponentials. `exp2(x) = 2^x`, `exp10(x) = 10^x`. C: `exp2` from C99 libm; `exp10` portably routed through `pow(10.0, x)` since it's a GNU extension not in C99. LLVM: declares `@exp2(double)`; `exp10` calls existing `@pow(double, double)` with `10.0` as the base. Both backends byte-identical: `exp2(0) = 1`, `exp2(1) = 2`, `exp2(10) = 1024`, `exp2(0.5) = √2 ≈ 1.41421`, `exp2(-1) = 0.5`; `exp10(0) = 1`, `exp10(1) = 10`, `exp10(3) = 1000`, `exp10(-2) = 0.01`, `exp10(0.5) = √10 ≈ 3.16228`. 1 new lib test. 1453 lib + 54 parity green. Closure #434 (numerical-stability helpers) shipped immediately before.)
