...
**Last updated:** 2026-06-01 (closures #512-#515 — **`vec_running_product`**, **`vec_running_xor`**, **`vec_running_and`**, **`vec_running_or`** (each `xs: ref Vec<i64> -> Vec<i64>`). Same structural shape as `vec_running_sum` (#398) and `vec_cumulative_max/min` (#510/#511) but parameterized over a different monoid: product (identity 1, op `*`), xor (id 0, op `^`), and (id `-1` all-ones, op `&`), or (id 0, op `|`). Single-pass with a running-best alloca; LLVM helper bodies emitted from one loop over `(op, ir_op, identity, prefix)` quadruples. Both backends byte-identical: with `xs=[2,3,4]`, `running_product=[2,6,24]`. With `xs=[1,2,4]`, `running_xor=[1,3,7]` and `running_or=[1,3,7]`. With `xs=[15,7,3]`, `running_and=[15,7,3]`. 1 new lib test. 1520 lib + 54 parity green.)
