...
**Last updated:** 2026-05-29 (closure #351 — **SipHash-2-4 keyed hash**: `siphash_i64(k0: u64, k1: u64, x: i64) -> u64` and `siphash_str(k0: u64, k1: u64, s: Str) -> u64` join the FNV-1a family (`hash_i64` / `hash_f64` / `hash_str` / `hash_combine`) with an adversarial-resistant alternative. Implementation is the canonical SipHash-2-4: 2 compression rounds per 8-byte block, 4 finalization rounds; initial state `(v0..v3) = constants XOR (k0,k1)` with the standard "somepseudorandomlygeneratedbytes" derivation. Both backends emit a shared `intent_siphash24_bytes(k0, k1, ptr, n)` core; thin `_i64` and `_str` wrappers cast / strlen as appropriate. **Spec-vector parity verified**: the published SipHash-2-4 vector for an empty message with key `0x000102…0f` is `0x726fdb47dd0e0e31` = `8246050544436514353`, and both backends emit exactly that value. C uses an `INTENT_SIP_ROUND` macro for compactness; LLVM uses a per-round-tagged IR generator (`emit_sipround_llvm`) so 8 chained rounds get unique SSA names. Hash-helper gate (LLVM `program_uses_siphash`, new walker) gates emission distinctly from FNV; C piggybacks on the loose `intent_siphash` substring check inside `emit_intent_hash_helpers_c`. Extended `examples/hash.vani` exercises determinism, key sensitivity (swapped halves), the spec vector, and `"vani"` vs `"rust"` distinction. 4 new lib tests pin typecheck (3-arg signatures), arity diagnostic, and helper-name emission in both backends. 1276 lib + 54 parity green across 90 examples. Closure #350 (`str_split`) shipped immediately before.)
