...
**Last updated:** 2026-05-29 (closure #333 — **Graph algorithm extensions: `has_cycle` / `mst_kruskal` / `mst_prim`** on the existing `Graph` type from closure #329, no new struct fields. Three new builtins layered on top of the existing per-edge parallel arrays: `graph_has_cycle(ref g) -> bool` uses Kahn's algorithm (in-degree counting + BFS, returns true iff not all nodes are peeled off in zero-in-degree order); `graph_mst_kruskal(ref g) -> Option<i64>` uses an insertion-sorted index array + path-compressed Union-Find (inlined within the helper, no dependency on `UnionFind` type) and returns the MST total weight or None if the graph is disconnected; `graph_mst_prim(ref g) -> Option<i64>` uses O(V²) linear-scan inner loop (no BinaryHeap dependency, parallel to the existing Dijkstra design) for the same answer. Both MST helpers treat edges as undirected (each directed (src,dst) pair contributes one u-v undirected edge with the recorded weight). Method sugar: `g.has_cycle()` / `g.mst_kruskal()` / `g.mst_prim()`. New `examples/graph_algo.vani` exercises DAG vs cycle detection, a small connected-weighted graph where MST should equal 6, a disconnected graph (both MSTs return None), and a single-node graph (MST weight = 0); both backends produce identical output. 4 new lib tests. Closure #332 (AVL on Bst) shipped immediately before. Roadmap: graph extensions are now well underway — remaining queued items are A* (path tracker on top of Dijkstra, needs closure-as-value type for the heuristic fn), topological sort returning a `Vec<i64>` order, CSR adjacency cache for O(V+E) BFS/DFS, plus the broader extensions Trie-alphabet / SkipList-remove / red-black-on-Bst that round out Level 4.)
**Test totals:** 1238 lib + 54 end-to-end + 11 vtables-phase3 + 2 user-drop-by-ref + 1 ssa-examples tests passing; the cross-backend parity runner covers all 88 examples under `examples/`. (Win32 LLVM dispatch adds 4 host-gated tests that fire on Windows hosts only — futex/WaitOnAddress, CreateThread for tasks, plus the CreateThread fan-out parallel-for tests in tree-LLVM and SSA-LLVM.)
