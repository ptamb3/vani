...
**Last updated:** 2026-05-28 (closure #326 — **Level 4 #2** `BinaryHeap<T>` dedicated wrapper type: promotes the Vec<i64>-backed heap (closure #302) to a first-class affine handle. New `BinaryHeap<T>` affine handle backed by `i64*` + `len` + `cap`. 5 builtins: `binary_heap_new()`, `binary_heap_push(mut ref h, v) -> i64` (sift-up, returns new len), `binary_heap_pop(mut ref h) -> Option<i64>` (extract-min + sift-down), `binary_heap_peek(ref h) -> Option<i64>`, `binary_heap_len(ref h) -> i64`. Method sugar: `h.push(v)` / `h.pop()` / `h.peek()` / `h.len()`. Scope-exit Drop frees the backing buffer; Option-returning helpers auto-monomorphize `Option<i64>` via the existing payload-registry walker. New `examples/binary_heap.vani` priority-queue drain demo; 6 new lib tests. Closure #325 (Level 4 #1) Union-Find shipped immediately before. Roadmap: **Level 4 is 2/9 complete** — next, Bloom filter (#327), BST/AVL node arena (#328), graphs + algorithms (#329), Trie (#330), skip list (#331).)
**Test totals:** 1202 lib + 54 end-to-end + 11 vtables-phase3 + 2 user-drop-by-ref + 1 ssa-examples tests passing; the cross-backend parity runner covers all 81 examples under `examples/`. (Win32 LLVM dispatch adds 4 host-gated tests that fire on Windows hosts only — futex/WaitOnAddress, CreateThread for tasks, plus the CreateThread fan-out parallel-for tests in tree-LLVM and SSA-LLVM.)
