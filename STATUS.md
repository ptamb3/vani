...
**Last updated:** 2026-05-30 (closure #390 — **`str_chars(s) -> Vec<i64>` + `str_reverse(s) -> OwnedStr`**. `str_chars` materializes the byte values [0..255] of `s` into a fresh `Vec<i64>` (one i64 per byte; zero-extended). `str_reverse` returns a fresh byte-reversed copy. Both are byte-level — UTF-8 codepoint awareness is deferred. C: `intent_str_chars` helper gated on `program_uses_graph_vec_builtin` since it returns `Vec<i64>`; `intent_str_reverse` always-on. LLVM: parallel defines. Method sugar `s.chars()` / `s.reverse()` registered alongside the other str method-dispatch entries. Both backends byte-identical: `"ABC".chars() = [65, 66, 67]`, `"hello".reverse() = "olleh"`. 2 new lib tests. 1381 lib + 54 parity green. Closure #389 (vec_take_while + vec_drop_while) shipped immediately before.)
