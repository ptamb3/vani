...
**Last updated:** 2026-05-31 (closure #433 — **`f64_erf / f64_erfc / f64_tgamma / f64_lgamma`** libm special functions. `erf(x)` is the Gauss error function; `erfc(x) = 1 - erf(x)` is the complementary form (more numerically stable for large positive x); `tgamma(x)` is the true gamma function (so `tgamma(n+1) = n!` for non-negative integers); `lgamma(x) = log(|gamma(x)|)` avoids overflow for large arguments where the gamma value itself wouldn't fit in a double. C: routed through libm directly. LLVM: declares `@erf`, `@erfc`, `@tgamma`, `@lgamma` and dispatches via a small `f64_*` → libm-name prefix-strip. Both backends byte-identical: `erf(0) = 0`, `erf(1) = 0.842701`, `erfc(0) = 1`, `erfc(1) = 0.157299` (= 1 - erf(1)), `tgamma(1) = 1` (= 0!), `tgamma(5) = 24` (= 4!), `tgamma(0.5) = 1.77245` (= √π), `lgamma(10) = 12.8018` (= log(9!) ≈ log(362880)), `lgamma(100) = 359.134`. 2 new lib tests. 1451 lib + 54 parity green. Closure #432 (relu/leaky_relu/softplus) shipped immediately before.)
