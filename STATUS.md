...
**Last updated:** 2026-05-31 (closure #404 — **boundary constants**: `i64_min_value() / i64_max_value() / f64_max_finite()`. Zero-arg builtins returning `INT64_MIN / INT64_MAX / DBL_MAX`. C: literal decimals for i64 limits + `1.7976931348623157e308` for DBL_MAX. LLVM: decimal literals for i64; `0x7FEFFFFFFFFFFFFF` (exact IEEE-754 bit pattern) for f64_max_finite. The checker short-circuits each before the F64 coerce loop with correct return types (I64 for the i64 limits, F64 for max_finite). Both backends byte-identical: `i64_min_value() = -9223372036854775808`, `i64_max_value() = 9223372036854775807`, `f64_is_finite(f64_max_finite()) = true`. 1 new lib test. 1402 lib + 54 parity green. Closure #403 (f64 bit access) shipped immediately before.)
