...
**Last updated:** 2026-05-29 (closure #345 — **Trie alphabet generalization (a-z → full u8)**: the per-node child stride changed from 26 to 256, dropping the lowercase-a-z restriction. Every nonzero u8 byte is now a valid character (mixed-case, punctuation, digits, all UTF-8 byte sequences). `intent_trie_valid_str` simplified to just a non-NULL check. Per-node memory grew from 104 bytes (26 × i32) to 1024 bytes (256 × i32); the LLVM realloc-size constant moved from `mul %cap_new, 104` to `mul %cap_new, 1024`. `sext i8 → i64` for character bytes became `zext i8 → i64` so bytes 128-255 stay positive in indexing. Extended `examples/trie.vani` exercises mixed-case + punctuation insertion (`"Hi!"` is no longer "invalid"); empty-string handling continues to work via the root node. Both backends produce byte-identical output. 1 new lib test pins the 256-wide stride (LLVM realloc constant). 1255 lib + 54 parity green across 90 examples. Closure #344 (Trie arena compaction) shipped immediately before. Future memory optimization: sparse per-node children via `Vec<(u8, u32)>` to bring per-node footprint back down — deferred behind closure work.)
