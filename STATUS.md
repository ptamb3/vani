...
**Last updated:** 2026-05-29 (closure #338 — **reverse-CSR for Prim's undirected adjacency**: completes the graph CSR story by adding a reverse adjacency cache that lets MST Prim iterate neighbors in O(degree) per pop instead of scanning all edges. Three new struct fields appended at indices 9/10/11: `rev_adj_start: i32*` (row pointers keyed by destination), `rev_adj_csr_src: i32*` (the source-node of each incoming edge), `rev_adj_csr_weight: i64*`. LLVM byte_size grows 72 → 96. A new `intent_graph_build_rev_csr_if_needed` helper does the same two-pass build as the forward CSR but keyed on destination: count in-degrees, prefix-sum, bucket-scatter. `intent_graph_invalidate_csr` now frees + nulls both caches. Prim builds both CSRs at entry, then for each popped `u` walks `adj_start[u]..adj_start[u+1]` (outgoing u→v) AND `rev_adj_start[u]..rev_adj_start[u+1]` (incoming v→u) — the union covers all edges incident to u under the undirected interpretation. With this change every graph algorithm except Kruskal (which is edge-sort-driven, not adjacency-driven) now does O(V+E) neighbor iteration. No API changes; existing examples produce byte-identical output across both backends. 1244 lib tests + parity green across 90 examples. Closure #337 (CSR adoption for has_cycle/topo_sort/dijkstra/astar) shipped immediately before.)
