...
**Last updated:** 2026-05-29 (closure #346 — **BTreeSet / BTreeMap range queries**: three new builtins on the existing sorted-Vec backings, each taking `(ref container, i64 lo, i64 hi, mut ref Vec<i64> out) -> i64` and appending every entry in `[lo, hi]` to `out` in sorted ascending order. `btreeset_range` (set keys), `btreemap_range_keys` and `btreemap_range_values` (parallel key/value emission — call both with the same lo/hi for paired output). Implementation reuses the existing `lower_bound` helper (binary-search), then walks forward until `keys[i] > hi`. O(log n + matches). Grows the output Vec by capacity doubling, matching the existing `topo_sort` "append-to-Vec" pattern. Helpers are gated on `intent_vec_int64_t` being emitted (extended `program_uses_graph_vec_builtin` to detect the new range names). Method sugar: `s.range(lo, hi, mut ref out)`, `m.range_keys(lo, hi, out)`, `m.range_values(lo, hi, out)`. Extended `examples/btreeset.vani` + `examples/btreemap.vani` exercise the helpers (range of bulk-inserted keys, empty range with no overlap, `lo>hi` returns 0). Both backends byte-identical. 5 new lib tests pin typecheck shape (3 args after the receiver), `mut ref Vec<i64>` enforcement on the output, and helper-name emission in both backends. 1260 lib + 54 parity green across 90 examples. Closure #345 (Trie alphabet generalization) shipped immediately before.)
