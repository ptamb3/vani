...
**Last updated:** 2026-05-31 (closure #454 — **`str_count_ascii_digits(s) -> i64`** count the decimal-digit bytes (`'0'..'9'` = `0x30..0x39`) in `s`. Useful for tokenization, validation, and quick numeric-content heuristics on text. Walks `s` until the null terminator with a `48 <= b <= 57` range check on each byte. C: inline GNU statement-expression with the for-walk + range check. LLVM: emits `@intent_str_count_ascii_digits(i8*) -> i64` helper alongside `@intent_str_byte_count` and `@intent_str_count_char` — uses `icmp uge i8 48` + `icmp ule i8 57` + `and i1` + `zext i1 → i64` for the per-byte contribution. Both backends byte-identical: `count('abc123def456') = 6`, `count('hello world') = 0`, `count('0123456789') = 10`, `count('') = 0`, `count('Phone: 555-1234') = 7`, `count('3.14159') = 6` (period not a digit), `count('//pure punctuation//') = 0`. 2 new lib tests. 1478 lib + 54 parity green. Closure #453 (log_b) shipped immediately before.)
