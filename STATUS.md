...
**Last updated:** 2026-05-31 (closure #424 — **`i64_pow_mod(a, b, m) -> i64`** modular exponentiation: computes `(a^b) mod m` via square-and-multiply with modular reduction at each step. Useful for number theory, cryptographic primitives, and modular arithmetic. Defensive defaults: returns 0 for `m <= 1`, `b < 0`. Normalizes `a` into `[0, m)` via `((a % m) + m) % m` before iterating, so negative `a` works correctly. Caveat: assumes `m * m` fits in i64 (i.e., `m < 2^31.5 ≈ 3.04e9`) — for larger m the `r*a` or `a*a` multiplications can overflow; caller must use a wider type for that case (documented in source). C: inline GNU statement-expression with the square-and-multiply loop. LLVM: emits a shared `@intent_i64_pow_mod(i64, i64, i64) -> i64` helper alongside `@intent_i64_log10_ceil`, with the same alloca-per-state + br-back-to-head loop shape as `@intent_i64_pow`. Both backends byte-identical: `2^10 mod 1000 = 24` (1024 mod 1000), `3^7 mod 100 = 87` (2187 mod 100), `5^0 mod 7 = 1`, `2^8 mod 13 = 9` (256 mod 13), `7^100 mod 13 = 9` (matches Fermat: 7^12 ≡ 1 mod 13, so 7^100 = 7^4 = 2401 = 9 mod 13), `2^62 mod (10^9+7) = 145586002`, `0^5 mod 7 = 0`, `5^5 mod 1 = 0` (everything mod 1), `2^-3 mod 7 = 0` (defensive default). 2 new lib tests. 1436 lib + 54 parity green. Closure #423 (log10_ceil) shipped immediately before.)
