...
**Last updated:** 2026-05-31 (closure #422 — **`i64_log10_floor(n) -> i64`** base-10 floor logarithm. Returns the largest k such that 10^k ≤ n; returns 0 for n ≤ 0 (defensive default — log10 of non-positive is undefined). Companion to the existing `i64_log2_floor` (closure #408) and `i64_count_digits` (#421) — note that `log10_floor(n) = count_digits(n) - 1` for n ≥ 1. C: inline GNU statement-expression with while-loop (`while n >= 10: n /= 10; c += 1`). LLVM: emits a shared `@intent_i64_log10_floor(i64) -> i64` helper alongside `@intent_i64_count_digits`, with the same alloca-per-state + br-back-to-head loop shape. Both backends byte-identical: `log10_floor(1) = 0`, `log10_floor(9) = 0`, `log10_floor(10) = 1`, `log10_floor(99) = 1`, `log10_floor(100) = 2`, `log10_floor(1000) = 3`, `log10_floor(0) = 0`, `log10_floor(-5) = 0`, `log10_floor(MAX) = 18` (10^18 ≤ INT64_MAX < 10^19). 2 new lib tests. 1432 lib + 54 parity green. Closure #421 (count_digits) shipped immediately before.)
