...
**Last updated:** 2026-05-31 (closure #455 — **`str_count_ascii_alpha(s) -> i64`** count ASCII alphabetic bytes (`'A'..'Z'` = `65..90`, `'a'..'z'` = `97..122`) in `s`. Companion to closure #454 (`str_count_ascii_digits`); part of the planned ASCII-class counter family. C: inline GNU statement-expression with two range checks and a logical OR. LLVM: emits `@intent_str_count_ascii_alpha(i8*) -> i64` helper — same walk-loop shape as `@intent_str_count_ascii_digits` but with `icmp uge 65 + icmp ule 90` (uppercase) OR `icmp uge 97 + icmp ule 122` (lowercase) as the per-byte predicate. Both backends byte-identical: `count('Hello, World!') = 10` (H,e,l,l,o + W,o,r,l,d), `count('abc123') = 3`, `count('HELLO') = 5`, `count('') = 0`, `count('123!@#') = 0`, `count('MixedCase42') = 9`. 1 new lib test. 1479 lib + 54 parity green. Closure #454 (digit-count) shipped immediately before.)
