...
**Last updated:** 2026-05-29 (closure #344 — **Trie arena compaction**: `trie_delete` now walks back up the path after clearing the terminal `is_end` bit and frees any node whose subtree has no end-of-word descendants. The `intent_trie` struct gains two fields — `free_head: i64` (init -1) and `free_count: i64` (init 0) — at indices 5 and 6 (byte_size 40 → 56, 7 LLVM fields). The freelist of recycled arena slots reuses `children[idx*26 + 0]` as a next-pointer (safe because freed nodes have all 26 children set to -1). `intent_trie_new_node` pops from the freelist first when available; otherwise it grows + appends at the high-water mark. `node_count()` returns `num_nodes - free_count` (live nodes only). Extended `examples/trie.vani` exercises full compaction (`apple` → delete → `node_count` drops from 6 to 1), freelist reuse (re-insert returns to 6), and partial compaction at a branch point (after `apple` + `app`, deleting `apple` reclaims only `l` and `e`; `a-p-p` stays because `app` is still a stored word). Both backends produce byte-identical output. 1 new lib test pins the 7-field LLVM struct + the C struct field names. 1254 lib + 54 parity green across 90 examples. Closure #343 (hashmap_remove via tombstones) shipped immediately before.)
