...
**Last updated:** 2026-05-29 (closures #334 + #335 — **A* with user-provided heuristic** + **topological sort writing into a `mut ref Vec<i64>`**, both on the existing `Graph` type, no new struct fields. (#334) `graph_astar(ref g, src, dst, ref h: Vec<i64>) -> Option<i64>` uses the same O(V²) linear-scan inner loop as Dijkstra but selects nodes by f = g + h instead of just g. The heuristic table is a plain `Vec<i64>` with one entry per node (admissibility is the caller's responsibility — a zero heuristic reduces A* to Dijkstra). Size-mismatched heuristics return None. (#335) `graph_topo_sort(ref g, mut ref out: Vec<i64>) -> i64` runs Kahn's algorithm and appends nodes to the user-provided Vec in topological order, returning the count appended (== num_nodes iff DAG, less on cycle). The helper grows `out` via realloc rather than calling `__push`, keeping the per-builtin gating logic simple. Both helpers are gated on actually being called (new `program_uses_graph_vec_builtin` walker) so plain Graph users don't get spurious `intent_vec_int64_t` references in their generated C. Method sugar: `g.astar(src, dst, ref h)` / `g.topo_sort(mut ref out)`. New `examples/graph_algo2.vani` exercises both — A* vs Dijkstra equivalence with zero heuristic, A* with an admissible heuristic, self-distance = 0, size-mismatched heuristic returns None, topo_sort on a DAG yields the count + order, topo_sort on a 3-cycle returns 0. 5 new lib tests. Closure #333 (has_cycle / mst_kruskal / mst_prim) shipped immediately before. Roadmap: graph extensions nearly complete — A* path tracking, CSR adjacency cache (#336), undirected variant, generic weights, the broader Trie/SkipList/RB extensions are what remains.)
**Test totals:** 1243 lib + 54 end-to-end + 11 vtables-phase3 + 2 user-drop-by-ref + 1 ssa-examples tests passing; the cross-backend parity runner covers all 89 examples under `examples/`. (Win32 LLVM dispatch adds 4 host-gated tests that fire on Windows hosts only — futex/WaitOnAddress, CreateThread for tasks, plus the CreateThread fan-out parallel-for tests in tree-LLVM and SSA-LLVM.)
