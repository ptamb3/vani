...
**Last updated:** 2026-05-31 (closure #456 — **`str_count_ascii_alphanumeric(s) -> i64`** count ASCII alphanumeric bytes (digit OR upper OR lower) in `s`. Continues the granular ASCII-class counter family (#454, #455). C: inline statement-expression with three range checks chained by `||`. LLVM: emits `@intent_str_count_ascii_alphanumeric(i8*) -> i64` helper — `icmp uge/ule` for each of the three byte ranges, joined with `or i1`, then `zext` to i64 for the per-iteration contribution. Both backends byte-identical: `count('Hello, World!') = 10` (alpha-only, comma/space excluded), `count('abc123') = 6` (3 alpha + 3 digit), `count('') = 0`, `count('!@#$%') = 0`, `count('Order#42_done!') = 11` (`#`, `_`, `!` excluded). 1 new lib test. 1480 lib + 54 parity green. Closure #455 (alpha-count) shipped immediately before.)
