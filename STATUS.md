...
**Last updated:** 2026-05-30 (closure #360 — **Option<f64> ergonomics**: `option_unwrap_or_f64`, `option_is_some_f64`, `option_is_none_f64`. Parallels the #357 i64 triad on the existing `Enum_Option__f64` struct (already plumbed for `parse_float`'s return type since #298). Eliminates the per-example `match Option.Some(v) then v, Option.None then default` boilerplate users were hand-writing around `parse_float` results. C helpers `intent_option_f64_unwrap_or` etc. emitted only when `Option__f64` is in the payload registry. LLVM mirrors via `@intent_option_f64_unwrap_or(%Enum_Option__f64, double)` — uses `extractvalue` on tag/payload + `select`. Both backends byte-identical. Auto-monomorphization walker extended so a bare `option_unwrap_or_f64(...)` call still forces the `Option__f64` decl even without explicit `parse_float` in scope. 3 new lib tests pin typecheck + helper-name emission + cross-type rejection (Option<i64> must not coerce to Option<f64>). 1300 lib + 54 parity green. Closure #359 (f64_to_str) shipped immediately before.)
