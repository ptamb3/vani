...
**Last updated:** 2026-05-31 (closure #398 — **`vec_running_sum(ref xs) -> Vec<i64>`** with method sugar `xs.running_sum()`. Cumulative sum: `result[i] = sum(xs[0..=i])`. C: helper in the Vec<i64> utility block with a running accumulator. LLVM: phi-driven loop with running accumulator alloca + per-iteration store-and-output. Method sugar `.running_sum()` registered alongside the other Vec methods. Both backends byte-identical: `[1,2,3,4,5].running_sum() = [1,3,6,10,15]`. 1 new lib test. 1392 lib + 54 parity green. Closure #397 (vec_zip_with) shipped immediately before.)
