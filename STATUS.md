...
**Last updated:** 2026-05-30 (closure #393 — **`i64_abs_diff(a, b) + i64_signum(x) + f64_signum(x)`** sign/abs-diff helpers. `i64_abs_diff` returns `|a - b|` via a `select` on the comparison (avoids signed-subtraction overflow on borderline values). `i64_signum(x)` returns -1 / 0 / +1 via `(x > 0) - (x < 0)`. `f64_signum(x)` returns -1.0 / 0.0 / +1.0; NaN stays NaN. C: inline statement-expressions. LLVM: explicit icmp/fcmp + zext/uitofp + sub sequences. `check_math_builtin` now produces Type::I64 for i64_signum (alongside the existing i64_gcd/lcm/pow / Bool classifiers / F64 family). 1 new lib test pins typecheck across all three. 1386 lib + 54 parity green. Closure #392 (vec_max_by + vec_min_by) shipped immediately before.)
