...
**Last updated:** 2026-05-31 (closure #416 — **`f64_copysign(mag, sign)`** + **`f64_fma(a, b, c)`** + **`f64_remainder(x, y)`** IEEE-754 math primitives. `f64_copysign(mag, sign)` returns `|mag|` with the sign bit of `sign`. `f64_fma(a, b, c)` returns `a*b + c` with a single rounding step (fused multiply-add) — more accurate than the separate-rounding form. `f64_remainder(x, y)` is the IEEE-754 remainder using round-to-even quotient (unlike `%` which uses truncation), so e.g. `remainder(7, 2) = -1`. C: emitted as direct calls to `copysign / fma / remainder` from `<math.h>`. LLVM: copysign and fma route through the `@llvm.copysign.f64` and `@llvm.fma.f64` intrinsics — these lower to a single hardware instruction on supported targets. Remainder routes through libm's `@remainder`. Both backends byte-identical: `copysign(5, -1) = -5`, `copysign(-5, 1) = 5`, `fma(2, 3, 1) = 7`, `fma(0.1, 10, 1) = 2` (fused rounding turns 0.1*10 into exactly 1.0), `remainder(10, 3) = 1`, `remainder(10, 4) = 2`, `remainder(7, 2) = -1` (round-to-even gives negative remainder). 2 new lib tests (typecheck + LLVM intrinsic + libm declare/call emission). 1421 lib + 54 parity green. Closure #415 (IEEE boundary constants) shipped immediately before.)
