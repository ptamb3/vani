...
**Last updated:** 2026-05-30 (closure #382 — **`vec_iota(n) -> Vec<i64>` + str method sugar for pad / lines / parse_bool**. `vec_iota(n)` is a specialization of `vec_range(0, n)` — generates `[0, 1, …, n-1]` (empty for non-positive `n`). Slightly tighter codegen than the general range form. C: `intent_vec_int64_t_iota` helper alongside the rest of the Vec<i64> utility family. LLVM: parallel define with phi-driven loop + per-iteration getelementptr + store. Also: method sugar entries `s.pad_left(n, ch)` / `s.pad_right(n, ch)` / `s.lines()` / `s.parse_bool()` added to the existing str method-dispatch table from #375, finishing the suite. Both backends byte-identical. 3 new lib tests pin typecheck + helper-name emission + method-sugar paths. 1365 lib + 54 parity green. Closure #381 (str_pad / str_lines) shipped immediately before.)
