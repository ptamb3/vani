...
**Last updated:** 2026-05-30 (closure #371 — **`vec_reverse_copy(ref xs) -> Vec<i64>` + `vec_unique(ref xs) -> Vec<i64>`**. Fresh-allocating variants of the existing in-place `reverse()` / `dedup()` mutators — the source Vec stays intact. `vec_reverse_copy` is a straight `memcpy` in reverse order; `vec_unique` is O(n²) walk-with-linear-scan that preserves first-occurrence order. C: helpers fall under `emit_intent_vec_int64_utility_helpers_c` alongside the rest of the family (gated on usage). LLVM: parallel defines with phi-driven outer/inner loops — the unique path uses a stacked phi for `out_len` and a separate inner-loop phi for the `seen` flag. Both backends byte-identical: `vec_reverse_copy([1,2,3,4,5]) = [5,4,3,2,1]`, `vec_unique([1,2,2,3,1,4,3]) = [1,2,3,4]`. 3 new lib tests pin typecheck + helper-name emission. 1335 lib + 54 parity green. Closure #370 (sort_desc) shipped immediately before.)
