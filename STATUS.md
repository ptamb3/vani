...
**Last updated:** 2026-05-30 (closure #357 — **`Option<i64>` ergonomics: `option_unwrap_or` / `option_is_some` / `option_is_none`**: eliminates the per-example `unwrap_or` boilerplate users were hand-writing. `option_unwrap_or(o, def) -> i64` returns the payload when `Some(_)` else `def`; `option_is_some`/`option_is_none` return bool. All three operate on the existing `Enum_Option__i64` struct (i32 tag + i64 payload). Gated on `Option__i64` being in the enum payload registry; auto-mono walker extended so any call to these helpers also registers `Option<i64>`. C uses ternary on `o.tag`; LLVM uses `extractvalue` + `select`. Both backends byte-identical. 2 new lib tests pin typecheck + helper-name emission. 1292 lib + 54 parity green. Closure #356 (Vec utility lump) shipped immediately before.)
