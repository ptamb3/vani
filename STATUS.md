...
**Last updated:** 2026-05-30 (closure #381 — **`str_pad_left(s, n, ch)` + `str_pad_right(s, n, ch) -> OwnedStr` + `str_lines(s) -> Vec<OwnedStr>`**. Three string helpers. The pad-left / pad-right helpers prepend / append the first byte of `ch` (empty `ch` defaults to space) until total length reaches `n`. If `s` is already at least `n` bytes, returns a fresh copy unchanged. `str_lines(s)` splits on `'\n'`, also stripping any trailing `'\r'` from each line for CRLF compatibility. C: dedicated `intent_str_pad_left/right` helpers always-on (`INTENT_UNUSED`-gated); `intent_str_lines` lives alongside `intent_str_split` (same gate). LLVM: padding via a loop-shaped define per direction; `intent_str_lines` wraps `@intent_str_split` and trims trailing CR in place. Both backends byte-identical. 3 new lib tests pin typecheck + helper-name emission. 1362 lib + 54 parity green. Closure #380 (i64 math triplet) shipped immediately before.)
