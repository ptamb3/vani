...
**Last updated:** 2026-05-31 (closure #449 — **`f64_l1_norm(a, b) -> f64`** + **`i64_isqrt_ceil(n) -> i64`** completing two related families. `l1_norm(a, b) = |a| + |b|` is the Manhattan / L^1 distance — completes the L^p trio alongside `f64_hypot` (#413, L^2) and `f64_chebyshev` (#445, L^∞). `isqrt_ceil(n) = ceil(sqrt(n))` is the ceiling integer-sqrt — companion to `i64_isqrt` (#412, floor variant). Algorithm: compute `s = floor_isqrt(n)`, then return `s` if `s*s == n` (perfect square) else `s + 1`. Returns 0 for `n <= 0`. C: `l1_norm` is `fabs(a) + fabs(b)` inline; `isqrt_ceil` is a longer inline GNU statement-expression inlining the Newton iteration + perfect-square test. LLVM: `l1_norm` uses two `call @fabs` + `fadd`; `isqrt_ceil` calls the shared `@intent_i64_isqrt` helper then `mul` + `icmp eq n` + chained `select` (with a guard for `n <= 0` returning 0). Both backends byte-identical: `l1_norm(3, 4) = 7`, `l1_norm(-3, 4) = 7`, `l1_norm(0, 0) = 0`, `l1_norm(-5, -12) = 17`; `isqrt_ceil(0) = 0`, `isqrt_ceil(1) = 1`, `isqrt_ceil(4) = 2` (exact), `isqrt_ceil(5) = 3` (round up), `isqrt_ceil(8) = 3`, `isqrt_ceil(9) = 3` (exact), `isqrt_ceil(10) = 4`, `isqrt_ceil(100) = 10`, `isqrt_ceil(101) = 11`, `isqrt_ceil(-7) = 0` (defensive). 1 new lib test. 1472 lib + 54 parity green. Closure #448 (f64_mod_floor) shipped immediately before.)
