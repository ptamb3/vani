...
**Last updated:** 2026-05-29 (closure #340 — **`trie_delete(mut ref t, s: Str) -> bool`**: removes an exact word from a `Trie` by walking to the terminal node and flipping its `is_end` bit. Returns true iff the word was present (false on absent words or any non-a-z input character — the underlying walker rejects). Arena slots stay tombstoned because other words may share their prefixes — a future closure could add a compaction pass for dead branches with no end-of-word descendants. Method sugar: `t.delete(s)`. New lib tests + extended `examples/trie.vani` exercise delete-present, delete-twice (returns false), delete-absent, and confirm that deleting "car" leaves "cart" intact (the prefix walk succeeds because the deleted node is still a valid arena slot on cart's path). Both backends produce byte-identical output. 1248 lib tests + parity green across 90 examples. Closure #339 (skiplist_remove) shipped immediately before.)
