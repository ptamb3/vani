...
**Last updated:** 2026-05-30 (closure #363 — **`log` / `log2` / `log10` / `exp` / `atan2` libm helpers**. Rounds out the math intrinsic surface alongside the existing `pow / sqrt / sin / cos / tan / floor / ceil / abs` builtins (#299). All f64-only, all libm-backed: `log` (natural log), `log2`, `log10`, `exp` (e^x), and `atan2(y, x)` (two-arg arctan). Single-arg helpers reuse the existing `sqrt | sin | ...` dispatch pattern (one-line addition to the `matches!` macro); `atan2` gets its own 2-arg dispatch arm parallel to `pow`. Checker math route extended; `want_args` updated to use 2 for `pow | atan2`. LLVM preamble grew 5 new `declare double @...` externs. SSA reject list extended so the SSA-LLVM bailout still routes through the tree backend for these. Both backends byte-identical: `exp(1)=2.71828`, `ln(e)=1`, `log2(8)=3`, `log10(1000)=3`, `atan2(1,1)=0.785398`. 3 new lib tests pin typecheck + libm call/declare emission + 2-arg arity check. 1311 lib + 54 parity green. Closure #362 (clamp + f64 min/max LLVM fix) shipped immediately before.)
