...
**Last updated:** 2026-05-30 (closure #385 — **`vec_first(ref xs) / vec_last(ref xs) -> Option<i64>`** with method sugar `xs.first() / xs.last()`. Trivial accessor pair returning `Some(xs[0])` / `Some(xs[len-1])` or `None` for empty Vec. C: dedicated `intent_vec_int64_t_first` / `intent_vec_int64_t_last` helpers in the Vec<i64> utility block (gated on Option<i64> being in the registry — the auto-mono walker now lists vec_first/vec_last alongside the other Option<i64>-forcing builtins). LLVM: parallel defines via a loop-generated pair (first/last differ only in the slot-index expression — 0 vs len-1). Method sugar entries `.first()` / `.last()` added to the Vec method-dispatch table. Both backends byte-identical: `[10,20,30].first() = Some(10)`, `.last() = Some(30)`, `[].first() = None`. 2 new lib tests pin typecheck + method sugar. 1372 lib + 54 parity green. Closure #384 (option_filter + option_or) shipped immediately before.)
