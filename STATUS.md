...
**Last updated:** 2026-05-31 (closure #436 — **`str_starts_with_byte(s, b) -> bool`** + **`str_ends_with_byte(s, b) -> bool`** byte-level prefix/suffix checks on `Str`. Useful for one-byte sentinels (e.g., quoting `'"'`, tabular line ends `'\n'`) where the existing string-based `str_starts_with` / `str_ends_with` would require allocating a single-character string. Both return `false` for empty strings (no first/last byte). C: inline GNU statement-expressions reading the relevant byte. LLVM: inline IR — `starts_with_byte` does a single `load i8` and `icmp`; `ends_with_byte` computes `strlen(s) - 1` then `getelementptr` + `load i8`. Both backends byte-identical: for `s = "Hello"`, `starts_with_byte(s, 72)` (H) `= true`, `starts_with_byte(s, 104)` (h) `= false`; `ends_with_byte(s, 111)` (o) `= true`, `ends_with_byte(s, 120)` (x) `= false`; `starts_with_byte("", 65) = false`, `ends_with_byte("", 65) = false`; for single-char `"A"`, both `starts_with_byte` and `ends_with_byte` return `true` against `65`. 1 new lib test. 1454 lib + 54 parity green. Closure #435 (exp2/exp10) shipped immediately before.)
