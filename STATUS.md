...
**Last updated:** 2026-05-30 (closure #396 — **`vec_replace_all(mut ref xs, old, new) -> i64`** with method sugar `xs.replace_all(old, new)`. Walks `xs` once, replacing every occurrence of `old` with `new` and returning the count of replacements. Pure in-place mutator — no allocation. C: inline statement-expression. LLVM: phi-driven loop with a counter alloca incremented on each match. Method sugar registered alongside `.swap / .remove_at` in the in-place Vec mutator family. Both backends byte-identical: `[1,2,3,2,1,2].replace_all(2, 99) = 3` (positions 1, 3, 5 swapped). 1 new lib test. 1389 lib + 54 parity green. Closures #394 + #395 (str_strip_prefix/suffix + str_count_char) shipped immediately before.)
