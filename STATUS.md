...
**Last updated:** 2026-05-29 (closure #327 — **Level 4 #6** `BloomFilter` probabilistic membership tester: fixed-size bit-array backing struct `{ u8* bits, num_bits, num_hashes, insert_count }`. 5 builtins: `bloom_filter_new(num_bits, num_hashes)`, `bloom_filter_insert(mut ref bf, x) -> i64` (sets `num_hashes` bits via double-hashing on FNV-1a; returns new insert count), `bloom_filter_contains(ref bf, x) -> bool`, `bloom_filter_len(ref bf) -> i64`, `bloom_filter_count(ref bf) -> i64`. Method sugar: `bf.insert(x)` / `bf.contains(x)` / `bf.len()` / `bf.count()`. Double-hashing: h1 = `intent_hash_i64`, h2 = a parallel-seed FNV mix; the LLVM hash helpers are now also gated on Bloom usage so the call site resolves. Scope-exit Drop frees the bit array. v1 keys are i64. False positives possible (probabilistic), false negatives impossible. New `examples/bloom_filter.vani` with insert/contains demo; 7 new lib tests. Closure #326 (Level 4 #2) BinaryHeap<T> shipped immediately before. Roadmap: **Level 4 is now 3/9 complete** — next, BST/AVL node arena (#328), graphs + algorithms (#329) (BFS/DFS/Dijkstra unblocked by BinaryHeap, Kruskal unblocked by UnionFind), Trie (#330), skip list (#331).)
**Test totals:** 1209 lib + 54 end-to-end + 11 vtables-phase3 + 2 user-drop-by-ref + 1 ssa-examples tests passing; the cross-backend parity runner covers all 82 examples under `examples/`. (Win32 LLVM dispatch adds 4 host-gated tests that fire on Windows hosts only — futex/WaitOnAddress, CreateThread for tasks, plus the CreateThread fan-out parallel-for tests in tree-LLVM and SSA-LLVM.)
