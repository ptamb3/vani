...
**Last updated:** 2026-05-31 (closure #429 — **`f64_sigmoid(x) -> f64`** + **`f64_softsign(x) -> f64`** neural-net activation functions. `sigmoid(x) = 1 / (1 + exp(-x))` — the classic logistic activation, range `(0, 1)`, midpoint `0.5` at `x = 0`. `softsign(x) = x / (1 + |x|)` — smooth sign-like activation, range `(-1, 1)`, computationally cheaper than `tanh`. C: inline GNU statement-expressions using libm `exp` and `fabs`. LLVM: inline IR — sigmoid uses `fsub` + `call @exp` + `fadd` + `fdiv`; softsign uses `call @fabs` + `fadd` + `fdiv`. Both backends byte-identical: `sigmoid(0) = 0.5`, `sigmoid(1) = 0.731059`, `sigmoid(-1) = 0.268941`, `sigmoid(10) = 0.999955`, `sigmoid(-10) = 4.53979e-05`; `softsign(0) = 0`, `softsign(1) = 0.5`, `softsign(-1) = -0.5`, `softsign(100) = 100/101 ≈ 0.990099`. 1 new lib test. 1445 lib + 54 parity green. Closure #428 (Fibonacci) shipped immediately before.)
