...
**Last updated:** 2026-05-31 (closure #427 — **`i64_factorial(n) -> i64`** saturating factorial. Returns `n!` for `n in [0, 20]`; saturates to `INT64_MAX` for `n > 20` (since `21! > 2^63 - 1`); returns 0 for negative n (defensive — factorial undefined for negative). Direct-multiplication algorithm in the in-range path; explicit branches for the saturation and defensive cases. C: inline GNU statement-expression. LLVM: emits a shared `@intent_i64_factorial(i64) -> i64` helper alongside `@intent_i64_is_prime`, with the same alloca-per-state + br-back-to-head loop shape. Both backends byte-identical: `0! = 1`, `1! = 1`, `2! = 2`, `5! = 120`, `10! = 3628800`, `15! = 1307674368000`, `20! = 2432902008176640000` (largest n! that fits in i64), `21! = INT64_MAX = 9223372036854775807` (saturates), `100! = INT64_MAX`, `(-5)! = 0` (defensive). 2 new lib tests. 1442 lib + 54 parity green. Closure #426 (str byte access) shipped immediately before.)
