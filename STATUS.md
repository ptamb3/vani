...
**Last updated:** 2026-05-30 (closure #376 — **Option<i64>/Option<f64> method-call sugar**. `o.unwrap_or(def)` / `o.is_some()` / `o.is_none()` desugar to the matching `option_*` builtin (with the `_f64` suffix when the inner type is f64). Dispatch on `Type::Enum("Option__i64")` vs `Type::Enum("Option__f64")` from the receiver Var's env lookup. Only `Var` receivers for now — non-trivial Option-producing expressions (like `parse_int("x").unwrap_or(0)`) need an explicit intermediate, same restriction as Vec/Str method sugar. No backend changes — the rewrite is in the checker's method-call dispatch (after the Str arm, before the Array arm) and just re-enters `check_call` with the builtin name. Both backends byte-identical. 2 new lib tests pin i64 + f64 method-sugar paths. 1347 lib + 54 parity green. Closure #375 (str method sugar) shipped immediately before.)
