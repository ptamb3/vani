...
**Last updated:** 2026-05-31 (closure #399 — **`vec_dot(ref xs, ref ys) -> i64`** with method sugar `xs.dot(ref ys)`. Pairwise multiply + accumulate; truncates to the shorter Vec. C: helper alongside the other vec utility ops with a running int64_t accumulator. LLVM: phi-driven loop with `min(xlen, ylen)` via `icmp ult + select` upfront, then per-iteration mul-and-accumulate into an alloca'd counter. Method sugar `.dot(ref ys)` registered in the Vec method-dispatch table. Both backends byte-identical: `[1,2,3].dot([10,20,30]) = 140`. 1 new lib test. 1393 lib + 54 parity green. Closure #398 (vec_running_sum) shipped immediately before.)
