...
**Last updated:** 2026-05-30 (closures #387 + #388 — **`vec_swap(mut ref xs, i, j) -> i64`** and **`vec_remove_at(mut ref xs, i) -> i64`**. `vec_swap` exchanges two elements by index; returns 0 (unit-style). `vec_remove_at` removes element at `i` while preserving order — shifts later elements left, decrements len, returns the removed value (complements existing `swap_remove` which is O(1) but doesn't preserve order). Both are in-place `mut ref Vec<i64>` mutators. C: inline statement-expressions touching `xs->data[]` / `xs->len` directly. LLVM: explicit getelementptr + load/store sequences (swap = 3-stage exchange; remove_at = phi-driven left-shift loop). Method sugar `xs.swap(i, j)` / `xs.remove_at(i)` registered alongside the other in-place Vec mutators. Both backends byte-identical. 3 new lib tests pin typecheck + method sugar + by-value rejection. 1377 lib + 54 parity green. Closure #386 (vec_count_if) shipped immediately before.)
