...
**Last updated:** 2026-05-30 (closure #379 — **`str_join(ref strs: Vec<OwnedStr>, sep: Str) -> OwnedStr`**. Concatenates the elements of `strs` with `sep` between them. Two-pass: sum lengths, allocate once, then memcpy each segment with the separator interspersed. Empty `strs` returns a fresh empty heap string; single-element strs returns just that element; empty `sep` joins with no separator. C: dedicated `intent_str_join` helper emitted alongside `intent_str_split` (both gated on `program_uses_str_split` which now also returns true for any `str_join` call — they share the `intent_vec_owned_str` dependency). LLVM: parallel `define i8* @intent_str_join(%intent_vec_i8p*, i8*)` with two-pass phi-driven loops + per-iteration getelementptr + memcpy. Both backends byte-identical: `str_join([foo,bar,baz], " | ") = "foo | bar | baz"`. 2 new lib tests pin typecheck + helper-name emission. 1355 lib + 54 parity green. Closure #378 (vec_position + closure-shorthand bool-inference) shipped immediately before.)
