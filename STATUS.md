...
**Last updated:** 2026-05-30 (closure #361 — **`bool_to_str(b: bool) -> OwnedStr`**: rounds out the to_str family alongside `i64_to_str` (#358) and `f64_to_str` (#359). Returns a malloc'd copy of `"true"` / `"false"` for symmetry with the other to_str helpers — the OwnedStr return path lets the result `+`-concatenate with other OwnedStr / Str. C helper added inside the existing `emit_intent_i64_to_str_c` emitter (`INTENT_UNUSED`-gated, no extra emission gating needed). LLVM mirrors via `define i8* @intent_bool_to_str(i1 %b)` — uses `select` on the boolean to pick between the existing `@.fmt.true` / `@.fmt.false` format globals and `memcpy`'s into a `malloc`'d buffer (5 bytes for "true", 6 for "false", both NUL-included). Both backends byte-identical: `true / false / has_two=true`. 3 new lib tests pin typecheck + helper-name emission + non-bool rejection. 1303 lib + 54 parity green. Closure #360 (Option<f64> ergonomics) shipped immediately before.)
