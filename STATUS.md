...
**Last updated:** 2026-05-29 (closure #329 — **Level 4 #5** `Graph` weighted directed graph + algorithms: backing struct `{ i64 num_nodes, i32* edge_src, i32* edge_dst, i64* edge_weight, i64 num_edges, i64 edge_capacity }` with per-edge parallel arrays (no CSR index — each algorithm scans all edges per visited vertex, fine for v1). 7 builtins: `graph_new(n)`, `graph_add_edge(mut ref, src, dst, w) -> i64`, `graph_num_nodes(ref) -> i64`, `graph_num_edges(ref) -> i64`, `graph_bfs_reach(ref, start) -> i64`, `graph_dfs_reach(ref, start) -> i64`, `graph_dijkstra(ref, src, dst) -> Option<i64>` (O(V²) inner loop — no BinaryHeap dependency; non-negative weights). Method sugar: `g.add_edge(s,d,w)` / `.num_nodes()` / `.num_edges()` / `.bfs_reach(s)` / `.dfs_reach(s)` / `.dijkstra(s,t)`. Scope-exit Drop frees the three edge arrays. New `examples/graph.vani` with BFS/DFS/Dijkstra demo on a small weighted DAG; 6 new lib tests. Closures #326 (BinaryHeap<T>), #327 (Bloom), and #328 (Bst<T>) shipped immediately before. Roadmap: **Level 4 is now 5/9 complete** — next, Trie (#330), skip list (#331), AVL/RB on the Bst arena, plus CSR-style adjacency cache on Graph + A* / topo / Kruskal / Prim algorithms.)
**Test totals:** 1221 lib + 54 end-to-end + 11 vtables-phase3 + 2 user-drop-by-ref + 1 ssa-examples tests passing; the cross-backend parity runner covers all 84 examples under `examples/`. (Win32 LLVM dispatch adds 4 host-gated tests that fire on Windows hosts only — futex/WaitOnAddress, CreateThread for tasks, plus the CreateThread fan-out parallel-for tests in tree-LLVM and SSA-LLVM.)
