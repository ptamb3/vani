...
**Last updated:** 2026-05-30 (closure #362 — **`clamp(x, lo, hi)` + f64 min/max LLVM fix**. Two-in-one. First, the new builtin: `clamp(x, lo, hi)` is a polymorphic intrinsic that returns `x` clipped to `[lo, hi]`. Two-step numeric-type promotion mirrors `min`/`max`, so `clamp(5, 0.0, 10)` lifts everything to f64. Lowering: C inlines a nested ternary `(x < lo ? lo : (x > hi ? hi : x))`; LLVM (both tree and SSA) emits two icmp/fcmp + select pairs. Wired across all four backends — tree-C, SSA-C, tree-LLVM, SSA-LLVM. Second, the latent bug fix: `min(a, b)` / `max(a, b)` on f64 was emitting invalid IR on tree-LLVM (`icmp slt` on doubles, referenced `@fn_min` which is undefined). On SSA-LLVM, min/max didn't have any dispatch arm at all — every direct primitive use produced an undefined `@fn_min` reference. Both paths now route floats through `fcmp olt`/`fcmp ogt` + `select`, and SSA-LLVM has a new dispatch arm that mirrors the SSA-C inline-ternary shape. User-shadow escape hatch: a non-3-arg `fn clamp(x: i64) -> i64` (used by the SMT verifier's `early_return_narrows_facts_on_fall_through` test) routes through the regular user-fn dispatch — checker / emit-paths all guard on arg count. 5 new lib tests pin clamp typecheck (i64 + f64), clamp arity-3 requirement, user-fn shadowing, and the f64-min/max fcmp-not-icmp lowering. 1308 lib + 54 parity green. Closure #361 (bool_to_str) shipped immediately before.)
