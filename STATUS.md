...
**Last updated:** 2026-05-31 (closure #410 — **`i64_saturating_add / sub / mul`** overflow-saturating arithmetic. Each clamps to `INT64_MIN` on negative overflow and `INT64_MAX` on positive overflow rather than wrapping. C: `__builtin_add_overflow / sub_overflow / mul_overflow` inside statement-expressions, with sign-based saturation selection (positive sum or product → `INT64_MAX`, else `INT64_MIN`). LLVM: emits `@llvm.sadd.sat.i64`, `@llvm.ssub.sat.i64`, and `@llvm.smul.fix.sat.i64(scale=0)` intrinsics — pure-integer fixed-point with scale 0 gives integer saturating multiply. Both backends byte-identical: `sat_add(MAX, 1)` and `sat_add(MAX, 1000)` both saturate to `9223372036854775807`; `sat_sub(MIN, 1)` saturates to `-9223372036854775808`; `sat_mul(MAX, 2)` → `MAX`; `sat_mul(MIN, 2)` → `MIN`; `sat_mul(MAX, -2)` → `MIN` (mixed signs); ordinary in-range operations pass through unchanged (`sat_add(3, 4) = 7`, `sat_mul(5, 6) = 30`). 2 new lib tests. 1409 lib + 54 parity green. Closure #409 (power-of-2 helpers) shipped immediately before.)
