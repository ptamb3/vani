...
**Last updated:** 2026-05-31 (closure #451 — **`f64_safe_log(x, default) -> f64`** + **`f64_geometric_mean(a, b) -> f64`** defensive log + geometric mean. `safe_log(x, default) = log(x)` for `x > 0`, else `default`. Companion to `f64_safe_sqrt` (#450) — guards against the log-of-non-positive trap (which returns `-Inf` for `x = 0` and `NaN` for `x < 0`). `geometric_mean(a, b) = sqrt(a * b)` with the convention that a negative product clamps to 0 (defensive — sqrt of negative would be NaN). Note: `geometric_mean(-4, -9) = 6` since the product `36` is positive — only mixed-sign inputs hit the defensive clamp. C: inline GNU statement-expressions with ternary guards. LLVM: `safe_log` uses `fcmp ogt 0` + `call @log` + `select`; `geometric_mean` uses `fmul` + `fcmp olt 0` + `call @sqrt` + `select`. Both backends byte-identical: `safe_log(1, -999) = 0` (= log(1)), `safe_log(e, 0) ≈ 0.999999` (= log(e) ≈ 1), `safe_log(0, -999) = -999`, `safe_log(-5, -999) = -999`, `safe_log(100, 0) = 4.60517`; `geom_mean(4, 9) = 6` (sqrt(36)), `geom_mean(1, 1) = 1`, `geom_mean(2, 8) = 4`, `geom_mean(0, 5) = 0`, `geom_mean(-4, 9) = 0` (defensive on negative product), `geom_mean(-4, -9) = 6` (product is +36). 1 new lib test. 1474 lib + 54 parity green. Closure #450 (safe_sqrt/safe_div) shipped immediately before.)
