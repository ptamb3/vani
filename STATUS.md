...
**Last updated:** 2026-05-30 (closure #377 — **`option_map(o, f) -> Option<i64>` + method sugar `o.map(f)`**. Functor-style map operation: applies `f: fn(i64) -> i64` to the Some payload (returning a new Option<i64>) or passes None through unchanged. C: inline statement-expression that branches on the tag, calls f on the payload, and rebuilds the Option. LLVM: explicit some/none branch with `extractvalue` for the payload, indirect call through the FnPtr, `insertvalue` for the result, then `phi` at the merge. Method sugar `o.map(f)` (only on Option<i64> today since f is currently i64→i64) added to the existing Option method-dispatch table from #376. Composes with the closure shorthand from #374: `o.map(|x| x * 2)` works end-to-end. Both backends byte-identical: `Some(1).map(|x| x*2) = Some(2)`; None passes through. 3 new lib tests. 1350 lib + 54 parity green. Closure #376 (Option method sugar) shipped immediately before.)
