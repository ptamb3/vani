...
**Last updated:** 2026-05-30 (closure #384 — **`option_filter(o, pred) -> Option<i64>` + `option_or(a, b) -> Option<i64>` + method sugar**. Two new Option combinators alongside the existing `option_map` / `option_unwrap_or` / `option_is_some/none` family. `option_filter` keeps `Some(v)` iff `pred(v)` is true; otherwise None. `option_or` is a first-Some-wins fallback (both arms always evaluate — no short-circuit semantics in v1). C: inline statement-expressions. LLVM: option_filter uses a 3-way branch (some-check / keep / none) with phi at the merge; option_or is a single `select` on the tag (no branching needed since both Option values are already materialized). Method sugar `o.filter(pred)` / `o.or(alt)` registered alongside `.map / .unwrap_or / .is_some / .is_none`. Both backends byte-identical. 3 new lib tests pin typecheck + arity check + method-sugar paths. 1370 lib + 54 parity green. Closure #383 (primitive .to_str() method sugar) shipped immediately before.)
