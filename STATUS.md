...
**Last updated:** 2026-05-31 (closure #443 — **`str_last_index_of_byte(s, b) -> Option<i64>`** last occurrence index of byte `b` in string `s`. Companion to closure #442's first-occurrence variant. Uses libc `strrchr` (= "string reverse char" — finds the last match). Useful for file-extension parsing (`str_last_index_of_byte("path/file.txt", 46)` → 8 — gives the position of the final `'.'`), path splitting, etc. Auto-triggers `Option<i64>` bundle via the existing walker. C: inline GNU statement-expression calling `strrchr`. LLVM: declares `@strrchr(i8*, i32)`. Dispatch reuses the same control-flow shape as `str_index_of_byte` — both share a unified arm that picks `strchr` vs `strrchr` based on the name. Both backends byte-identical: for `s = "hello world"`, `last_index_of_byte('l') = 9` (positions 2, 3, 9), `last_index_of_byte('o') = 7` (4 and 7), `last_index_of_byte(' ') = 5`, `last_index_of_byte('d') = 10`, `last_index_of_byte('h') = 0`, `last_index_of_byte('x') = None`; for `"a.b.c.txt"`, `last_index_of_byte('.') = 5` (perfect for extension extraction); for `"aaaa"`, `last_index_of_byte('a') = 3`; empty string → `None`. 2 new lib tests. 1463 lib + 54 parity green. Closure #442 (first-byte-index) shipped immediately before.)
