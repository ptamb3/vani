...
**Last updated:** 2026-05-30 (closure #364 — **`f64_is_nan` / `f64_is_inf` / `f64_is_finite` float classification**. Three bool-returning float predicates that round out the f64 numerical surface alongside the log/exp/atan2 family from #363. C: uses the existing `<math.h>` macros (`isnan` / `isinf` / `isfinite`) wrapped in `(... != 0)` to give them explicit bool type. LLVM: inlined via fcmp — no new libm extern needed. `f64_is_nan(x)` is `fcmp uno double x, x` (NaN is unordered against itself); `f64_is_inf(x)` and `f64_is_finite(x)` both `fabs(x)` first and then compare against the +Inf bit pattern `0x7FF0000000000000` (oeq for inf, olt for finite). The check_math_builtin path now produces `Type::Bool` for the three classification helpers — same coerce-args-to-f64 path as the rest of the math family, so `f64_is_nan(42)` (passing i64) coerces silently and returns false. Both backends byte-identical on all six classification cases (finite, +inf, -inf, nan, 0, positive). 3 new lib tests pin typecheck + isnan/isinf/isfinite C-macro emission + fcmp-uno + fabs LLVM emission. 1314 lib + 54 parity green. Closure #363 (log/exp/atan2 family) shipped immediately before.)
