...
**Last updated:** 2026-05-31 (closure #447 — **`i64_perm(n, k) -> i64`** permutation count `P(n, k) = n! / (n-k)! = n * (n-1) * ... * (n-k+1)`. Companion to closure #431 (`i64_binomial`). Iterative product with overflow detection — saturates to `INT64_MAX` when the running product would overflow (via `__builtin_mul_overflow` in C and `@llvm.smul.with.overflow.i64` in LLVM). Defensive defaults: returns 0 for `k < 0`, `n < 0`, or `k > n` (no permutation exists), and 1 for `k == 0`. C: inline GNU statement-expression with the overflow-checked loop. LLVM: emits a shared `@intent_i64_perm(i64, i64) -> i64` helper alongside `@intent_i64_binomial`, with the same alloca-per-state + br-back-to-head loop shape. Both backends byte-identical: `P(5, 0) = 1`, `P(5, 1) = 5`, `P(5, 2) = 20`, `P(5, 3) = 60`, `P(5, 5) = 120 = 5!`, `P(10, 3) = 720`, `P(20, 5) = 1860480`; defensive cases: `P(5, 6) = 0`, `P(5, -1) = 0`, `P(-5, 2) = 0`; `P(100, 50) = INT64_MAX` (overflows during accumulation, saturates). 2 new lib tests. 1470 lib + 54 parity green. Closure #446 (i64_wrap/f64_wrap) shipped immediately before.)
