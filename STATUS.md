...
**Last updated:** 2026-05-30 (closure #375 — **Str/OwnedStr method-call sugar**. `s.contains("x")` / `s.starts_with("h")` / `s.ends_with("!")` / `s.trim()` / `s.replace(from, to)` / `s.split(",")` / `s.index_of("x")` / `s.repeat(3)` / `s.to_upper()` / `s.to_lower()` / `s.substring(0, 5)` all desugar to the corresponding `str_*` builtin (or `substring` for the un-prefixed one). Accepts both `Var` receivers (`s.trim()`) and string-literal receivers (`"abc".repeat(3)`) — the literal case is detected by `ExprKind::Str(_)` directly since the env-lookup path would miss it. No backend changes — the rewrite happens in the checker's method-call dispatch before reaching `check_call`. Both backends byte-identical. 3 new lib tests pin var-receiver, literal-receiver, and Option-returning paths. 1345 lib + 54 parity green. Closure #374 (anonymous-fn shorthand `|x| ...`) shipped immediately before.)
