...
**Last updated:** 2026-05-29 (closure #349 — **`str_replace(s: Str, from: Str, to: Str) -> OwnedStr`**: second heap-allocating string builtin after #348's `str_trim`. Replaces every non-overlapping occurrence of `from` in `s` with `to`, returning a fresh OwnedStr handle. Two-pass implementation: walk via `strstr` to count matches and size the buffer exactly, then walk again copying spans + replacements. Edge cases: empty `from` is a no-op (a matching loop would otherwise diverge — hand back a fresh dup of `s`); empty `to` deletes every occurrence; no-match returns a fresh dup; empty input returns a fresh empty buffer; UTF-8 byte sequences in the input and substitution preserve correctly (`vāṇī ❤ rust` round-trips with `❤` → `<3`). C helper uses libc `strstr` + `memcpy`; LLVM mirrors it in IR with two scan loops, `ptrtoint` for span arithmetic, and per-step `memcpy` calls. Helpers emitted unconditionally (matching `intent_str_concat` / `intent_str_trim`). Extended `examples/string_ops.vani` covers single match, growing replace (`a`→`XYZ`), shrinking replace (`foo`→`FB`), delete (`X`→`""`), empty-`from` no-op, no-match, empty input. Both backends byte-identical. 3 new lib tests pin typecheck shape (3 args → OwnedStr), arity diagnostic, helper-name emission. 1269 lib + 54 parity green across 90 examples. Closure #348 (`str_trim`) shipped immediately before.)
