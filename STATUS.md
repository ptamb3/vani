...
**Last updated:** 2026-05-31 (closure #446 — **`i64_wrap(x, lo, hi) -> i64`** + **`f64_wrap(x, lo, hi) -> f64`** wrap values into the half-open interval `[lo, hi)` via floor-mod. Useful for cyclic indexing (toroidal grids), angle normalization (`f64_wrap(angle, -π, π)`), and any modular arithmetic where C's `%` (sign-of-dividend) gives the wrong semantics. Empty-range case (`hi <= lo`) returns `x` unchanged (identity / no-op). Algorithm: `m = (x - lo) mod (hi - lo)`; if `m < 0`, add `(hi - lo)` to lift it back into `[0, range)`; return `lo + m`. C: inline GNU statement-expressions using `%` (i64) / `fmod` (f64). LLVM: emits shared `@intent_i64_wrap(i64, i64, i64) -> i64` and `@intent_f64_wrap(double, double, double) -> double` helpers — both use a leading branch on empty range, then `srem`/`fmod` + `select` for the sign-fixup, then add `lo` back in. Both backends byte-identical: `i64_wrap(5, 0, 10) = 5` (in range), `i64_wrap(15, 0, 10) = 5` (wraps once), `i64_wrap(-3, 0, 10) = 7` (negative wraps up), `i64_wrap(10, 0, 10) = 0` (upper bound wraps to lower), `i64_wrap(7, 0, 0) = 7` (empty-range identity), `i64_wrap(25, -5, 5) = -5`; `f64_wrap(0.5, 0, 1) = 0.5`, `f64_wrap(1.5, 0, 1) = 0.5`, `f64_wrap(-0.5, 0, 1) = 0.5`, `f64_wrap(π, -π, π) = -π` (upper-bound wraps — same direction in angle convention), `f64_wrap(7π, -π, π) = -π`. 2 new lib tests. 1468 lib + 54 parity green. Closure #445 (inv_lerp/chebyshev) shipped immediately before.)
