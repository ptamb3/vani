...
**Last updated:** 2026-06-01 (closures #510/#511 — **`vec_cumulative_max(xs: ref Vec<i64>) -> Vec<i64>`** and **`vec_cumulative_min(xs: ref Vec<i64>) -> Vec<i64>`** running-extremum reductions returning a fresh same-length `Vec<i64>`. `result[i] = extremum(xs[0..=i])`. Mirror of `vec_running_sum` (#398) but for max / min instead of sum. Single-pass with a running-best alloca; both backends `malloc` a fresh i64* buffer + wrap in `intent_vec_i64`. Both backends byte-identical: with `xs = [3,1,4,1,5,9,2,6]`, `cumulative_max = [3,3,4,4,5,9,9,9]`, `cumulative_min = [3,1,1,1,1,1,1,1]`; empty Vec → empty result (null data ptr, len=0). 1 new lib test. 1519 lib + 54 parity green.)
