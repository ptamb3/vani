...
**Last updated:** 2026-05-31 (closure #452 — **`f64_harmonic_mean(a, b)`** + **`f64_quadratic_mean(a, b)`** completing the four classical Pythagorean / Pythagorean-related means alongside the arithmetic mean (= `f64_lerp(a, b, 0.5)`) and geometric mean (#451). `harmonic_mean(a, b) = 2ab / (a + b)` — the standard for averaging rates/ratios where the reciprocals are linear (e.g., average speed over equal distances). Returns 0 when `a + b = 0` (defensive). `quadratic_mean(a, b) = sqrt((a² + b²) / 2)` — also called root-mean-square (RMS) of two values. Useful for signal magnitudes, statistical spreads, and the AM-QM inequality. C: inline GNU statement-expressions. LLVM: `harmonic_mean` uses `fadd` + `fcmp oeq 0` + `fmul` + `fmul 2.0` + `fdiv` + `select` for the divide-by-zero guard. `quadratic_mean` uses two `fmul` (squares) + `fadd` + `fdiv 2.0` + `call @sqrt`. Both backends byte-identical: `harm(4, 4) = 4` (a = b → harm = a), `harm(2, 6) = 3` (= 24/8), `harm(1, 1000) = 1.998` (heavily weighted toward the smaller value), `harm(5, -5) = 0` (sum is 0, defensive), `harm(0, 5) = 0`; `quad(3, 4) = sqrt(12.5) ≈ 3.53553`, `quad(0, 0) = 0`, `quad(5, -5) = 5` (sign cancels in squaring), `quad(1, 1) = 1`. 1 new lib test. 1475 lib + 54 parity green. Closure #451 (safe_log/geom_mean) shipped immediately before.)
