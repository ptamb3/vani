...
**Last updated:** 2026-05-30 (closure #383 — **primitive `.to_str()` method sugar for `bool`/`i64`/`f64`**. `b.to_str()` desugars to `bool_to_str(b)`; `n.to_str()` → `i64_to_str(n)`; `x.to_str()` → `f64_to_str(x)`. Dispatch on the receiver's inferred type — bool / signed-int (i8/i16/i32/i64) / float (f32/f64) — and also accepts bare literal receivers (`true.to_str()`, `42.to_str()`, `3.14.to_str()`). No backend changes — rewrite in the checker's method-dispatch arm. The method-call sugar lands before the existing Vec/Str/Option arms in the same dispatch flow. Both backends byte-identical. 2 new lib tests pin var-receiver + literal-receiver paths across all three primitive types. 1367 lib + 54 parity green. Closure #382 (vec_iota + str method sugar) shipped immediately before.)
