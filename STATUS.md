...
**Last updated:** 2026-05-30 (closure #386 — **`vec_count_if(ref xs, pred) -> i64`** with method sugar `xs.count_if(pred)`. Counts elements where pred(x) is true; same predicate shape `fn(i64) -> bool` as `vec_filter` / `vec_position`. C: dedicated `intent_vec_int64_t__count_if` helper alongside the other per-Vec reductions (sum/product/min/max/count/any/all/chain), always-on (no Option dependency). LLVM: parallel define with phi-driven loop accumulating into an alloca'd counter; `zext i1 to i64` adds 0 or 1 per element. Method sugar `.count_if(pred)` registered in the Vec method-dispatch table. Both backends byte-identical: `[1..10].count_if(|x| x % 2 == 0) = 5`. 2 new lib tests pin typecheck + method sugar. 1374 lib + 54 parity green. Closure #385 (vec_first + vec_last) shipped immediately before.)
