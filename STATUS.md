...
**Last updated:** 2026-05-30 (closure #374 — **Anonymous-fn shorthand `|x| x + 1`**. Surface-syntax sugar that parses as a primary-position `|` (parallel to the existing `fn(...) -> ... { ... }` long form). `|x| x * 2` desugars to `AnonFn { params: [x:i64], return_type: i64, body: [return x * 2;] }`; `|a, b| body` for two params, comma-separated. All parameters and return type default to `i64` — matches the v1 closures surface (everything is i64). The existing lambda-lift pass picks up these AnonFn nodes and hoists them into `__anon_fn_<N>` top-level functions; no codegen changes were needed. Disambiguation with bitwise `|`: at primary position `|` always starts a closure; `a | b` is an infix operator and never appears in primary leader. Empty params `||` would lex as `OrOr` (so it's naturally unreachable through the shorthand path). Both backends byte-identical for `vec_map`, `sort_by`, `vec_fold` with closure shorthand. 3 new lib tests pin single-param + two-param shapes + bitwise-OR regression guard. 1342 lib + 54 parity green. Closure #373 (parse_bool) shipped immediately before.)
