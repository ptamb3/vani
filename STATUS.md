...
**Last updated:** 2026-05-29 (closure #341 — **SkipList tail tracker**: adds a single `i64 tail_node` field at struct index 7 (byte_size 56 → 64) — the index of the rightmost (highest-key) node in the list, or -1 if empty. Insert maintains it: after placing a new node, if the new node's level-0 forward is -1 it's the new tail. Remove maintains it: if the removed node was the tail, the new tail is `update[0]` (or -1 if `update[0]` is the head sentinel, meaning the list is now empty). `max()` now reads `keys[tail_node]` directly in O(1) instead of doing an O(n) level-0 walk to the end. Existing `examples/skiplist.vani` produces byte-identical output to before — the optimization is observation-free. 1 new lib test pins the 8-field LLVM struct shape to catch future field-order regressions. 1249 lib tests + parity green across 90 examples. Closure #340 (trie_delete) shipped immediately before.)
