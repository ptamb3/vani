...
**Last updated:** 2026-05-31 (closure #407 — **set ops on Vec<i64>**: `vec_intersect / vec_difference / vec_union(ref xs, ref ys) -> Vec<i64>`. All three return a fresh deduplicated Vec<i64>. Implementation is O(n*m) linear-scan dedup — fine for the v1 i64-only audience; sorted-Vec variants can ship later if needed. C: dedicated helpers using a shared `intent_vec_int64_t_contains_value` linear-scan predicate. LLVM: shared `@intent_vec_int64_t_member_scan(%intent_vec_i64*, i64) -> i1` helper + three define bodies generated from a `(helper_op, intersect_with_ys, difference_against_ys, include_ys_second_pass)` tuple loop. Method sugar `.intersect / .difference / .union(ref ys)` registered in the Vec method-dispatch table. Both backends byte-identical: `[1..5].intersect([3..7]) = [3,4,5]`, `[1..5].difference([3..7]) = [1,2]`, `[1..5].union([3..7]) = [1..7]`, and dedup verified on duplicate inputs. 1 new lib test. 1405 lib + 54 parity green. Closure #406 (f64_lerp + f64_clamp01) shipped immediately before.)
