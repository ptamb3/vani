...
**Last updated:** 2026-05-30 (closure #380 — **`i64_gcd(a, b)` + `i64_lcm(a, b)` + `i64_pow(base, exp)`**. Integer math triplet returning `i64`. `i64_gcd` uses Euclidean iteration after absolutizing both args. `i64_lcm` = `(|a|/gcd(|a|,|b|)) * |b|`, with `0` when either arg is `0`. `i64_pow` uses fast-exponentiation by squaring; negative `exp` returns `0` by convention. C: inline GNU statement-expressions (the gcd/pow forms produce expressions naturally; lcm needed a result-var lift to keep the trailing if/else from being a void-context statement). LLVM: dedicated `@intent_i64_gcd` / `@intent_i64_lcm` / `@intent_i64_pow` defines emitted unconditionally in the preamble — LLVM DCE drops unused ones at link time. `check_math_builtin` produces `Type::I64` for these three, alongside the existing `Type::Bool` / `Type::F64` branches. Both backends byte-identical across positive / negative / zero / large-exponent cases. 4 new lib tests pin typecheck + negative-exp / negative-arg semantics + LLVM define emission. 1359 lib + 54 parity green. Closure #379 (str_join) shipped immediately before.)
