...
**Last updated:** 2026-05-30 (closure #372 — **`f64_round(x) -> i64` + `f64_trunc_to_i64(x) -> i64`**. Two float-to-int helpers. `f64_round` uses round-half-away-from-zero semantics (libc `llround` on C, LLVM `@llround` extern on the other backend). `f64_trunc_to_i64` is a truncating-toward-zero conversion (C `(int64_t)x` cast on one side, LLVM `fptosi double … to i64` on the other). `check_math_builtin` now produces `Type::I64` for these (alongside `Type::Bool` for the classification helpers and `Type::F64` for the rest). 1 new libm extern declared in the LLVM preamble. Both backends byte-identical on all rounding cases (positive/negative/half/truncation/zero). 2 new lib tests pin typecheck + emission shape. 1337 lib + 54 parity green. Closure #371 (vec_reverse_copy + vec_unique) shipped immediately before.)
