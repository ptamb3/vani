...
**Last updated:** 2026-05-31 (closure #402 — **byte-swap + rotate** primitives: `i64_bswap(x)` (reverse 8 bytes), `i64_rotate_left(x, n)`, `i64_rotate_right(x, n)`. C: routes through `__builtin_bswap64` + the canonical `(x << n) | (x >> (64 - n))` rotate idiom with shift-count masking to `[0..63]` and a special-case branch for n == 0 to avoid the (64 - 0) shift-by-64 UB. LLVM: routes through `@llvm.bswap.i64 / @llvm.fshl.i64 / @llvm.fshr.i64` funnel-shift intrinsics declared in the preamble — for pure rotate, fshl(x, x, n) is the canonical encoding. Both backends byte-identical: `bswap(1) = 0x0100000000000000`, `rotl(1, 4) = 16`, `rotl(1, 64) = 1` (full rotation), `rotr(1, 1) = 0x8000000000000000`. 2 new lib tests. 1399 lib + 54 parity green. Closure #401 (popcount + clz + ctz) shipped immediately before.)
