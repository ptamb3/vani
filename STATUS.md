...
**Last updated:** 2026-05-30 (closure #366 — **`substring(s: Str, start: i64, len: i64) -> OwnedStr`**. Returns a freshly-malloc'd copy of bytes `[start, start+len)` from `s`. Three-way out-of-bounds clamping: negative `start` / `len` clamp to 0; `start > strlen(s)` clamps `start` to `strlen(s)`; `start + len > strlen(s)` truncates `len` to fit. NULL `s` is treated as empty. C: dedicated `intent_substring` helper emitted unconditionally with the rest of the str family. LLVM: dedicated `define i8* @intent_substring(i8*, i64, i64)` with `phi`-based strlen-or-zero handling for NULL, `select`-based clamping, and `getelementptr` + `memcpy` for the byte copy. Both backends byte-identical across in-bounds (`substring("hello, world", 0, 5) = "hello"`), over-shoot (`substring(s, 0, 100) = "hello, world"`), past-end (`substring(s, 99, 5) = ""`), zero-len, and negative-start cases. 3 new lib tests pin typecheck + helper-name emission + arity-3 requirement. 1320 lib + 54 parity green. Closure #365 (str_index_of) shipped immediately before.)
