...
**Last updated:** 2026-05-31 (closure #448 — **`f64_mod_floor(x, y) -> f64`** Python-style float modulo. Returns `x - y * floor(x / y)` — sign of result matches sign of divisor `y`, unlike libm `fmod` which gives sign-of-dividend remainder. Returns 0 for `y == 0` (defensive). Companion to closure #405's `i64_mod_floor`. Useful for cyclic-quantity arithmetic where wrapping behavior must match the natural interpretation (e.g., angle phase ≡ phase mod 2π should land in `[0, 2π)` regardless of input sign). C: inline GNU statement-expression — `y == 0.0 ? 0.0 : (x - y * floor(x / y))`. LLVM: `fcmp oeq y, 0` for the zero guard + `fdiv` + `call @floor` + `fmul` + `fsub`, then `select` to clamp the zero-divisor case to 0. Both backends byte-identical: `mod_floor(7, 3) = 1`, `mod_floor(-7, 3) = 2` (sign of y wins), `mod_floor(7, -3) = -2`, `mod_floor(-7, -3) = -1`, `mod_floor(10.5, 2.5) = 0.5`, `mod_floor(0, 5) = 0`, `mod_floor(5, 0) = 0` (defensive), `mod_floor(π, 2) = π - 2 ≈ 1.14159`, `mod_floor(-0.5, 1) = 0.5` (negative wraps positive). 1 new lib test. 1471 lib + 54 parity green. Closure #447 (i64_perm) shipped immediately before.)
