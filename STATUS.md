...
**Last updated:** 2026-05-31 (closure #442 — **`str_index_of_byte(s, b) -> Option<i64>`** first occurrence index of byte `b` in string `s`. Returns `Some(i)` if found, `None` if not. Single-byte counterpart to `str_index_of(haystack, needle)` (which uses `strstr` for string-needle); this version uses `strchr` for the cheaper single-byte lookup. Auto-triggers the `Option<i64>` enum bundle through the existing `walk_expr_for_search_builtins` gating. C: inline GNU statement-expression that calls `strchr` and packs the (offset or None) into `Enum_Option__i64`. LLVM: declares `@strchr(i8*, i32)`, then mirrors the `str_index_of` IR shape — `trunc i64 → i32` for the byte, `call @strchr`, `icmp eq null` to branch into Some/None labels, `ptrtoint`+`sub` for the offset, and a `phi` to merge the result. Both backends byte-identical: for `s = "hello world"`, `index_of_byte('o') = 4`, `index_of_byte(' ') = 5`, `index_of_byte('h') = 0`, `index_of_byte('d') = 10`, `index_of_byte('x') = None`; for `"a,b,c"`, `index_of_byte(',') = 1`; empty string yields `None` for any byte. 1 new lib test. 1461 lib + 54 parity green. Closure #441 (byte_count) shipped immediately before.)
