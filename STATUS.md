...
**Last updated:** 2026-05-31 (closure #420 — **`f64_trunc(x) -> f64`** + **`f64_frac(x) -> f64`** float truncation and fractional part. `f64_trunc(x)` rounds toward zero, returning the integer-valued double — e.g. `trunc(3.7) = 3`, `trunc(-3.7) = -3` (toward zero, not toward -inf). `f64_frac(x)` returns the signed fractional part such that `x = trunc(x) + frac(x)`. For positive x: `frac(3.7) = 0.7`; for negative x: `frac(-3.7) = -0.7` (preserves sign — so `frac` is not always in `[0, 1)`). C: routed through libm's `trunc()` directly; `frac` is `x - trunc(x)`. LLVM: declares `@trunc(double)` and routes `f64_trunc` through it; `f64_frac` emits `call @trunc` + `fsub`. Both backends byte-identical: `trunc(3.7) = 3`, `trunc(-3.7) = -3`, `trunc(3.0) = 3` (exact), `frac(3.7) = 0.7`, `frac(-3.7) = -0.7`, `frac(0.25) = 0.25`. 2 new lib tests (typecheck + `@trunc` declare + trunc/fsub shape). 1428 lib + 54 parity green. Closure #419 (int div companions) shipped immediately before.)
