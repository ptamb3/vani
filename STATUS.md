...
**Last updated:** 2026-05-29 (closure #339 — **`skiplist_remove(mut ref sl, x) -> bool`**: standard skip-list removal. Walks down levels MAX_LEVEL-1..0 recording an `update[]` array of nodes whose forward pointer might need to skip past the candidate, then redirects every level-l forward in `update[]` that pointed at the candidate to point at the candidate's own forward instead. The removed slot stays tombstoned in the arena (no compaction; parallel to Bst remove semantics). Returns true iff a node was removed; false if the candidate wasn't present. Method sugar: `sl.remove(x)`. New lib tests + extended `examples/skiplist.vani` exercise both removing a present key (followed by min/max/contains sanity), removing an absent key (returns false), and draining the entire skip list by repeated removal. Both backends produce byte-identical output. 1246 lib tests + parity green across 90 examples. Closure #338 (reverse-CSR for Prim) shipped immediately before — graph CSR story complete.)
