...
**Last updated:** 2026-05-30 (closure #373 — **`parse_bool(s: Str) -> Option<bool>`**. Recognizes `"true"` / `"false"` exactly (case-sensitive); anything else is `Option.None`. Introduces the `Option<bool>` enum monomorph — the auto-mono walker (`needed_arg: Some(Type::Bool)` for parse_bool) registers the requirement so the generic enum-emission code in both backends produces `Enum_Option__bool { i32 tag; bool payload }` (C) / `%Enum_Option__bool = type { i32, i1 }` (LLVM). C: inline statement-expression that calls `strcmp(s, "true")` / `strcmp(s, "false")` and packs the tag/payload. LLVM: explicit 3-way branch (some-true / some-false / none) with `insertvalue` on `i1 true` or `i1 false` payloads — reuses the existing `@.fmt.true` / `@.fmt.false` globals as the strcmp string literals. Both backends byte-identical across `"true"`, `"false"`, and unrecognized inputs. 2 new lib tests pin typecheck + enum-decl emission. 1339 lib + 54 parity green. Closure #372 (f64_round + f64_trunc_to_i64) shipped immediately before.)
