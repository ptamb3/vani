...
**Last updated:** 2026-05-29 (closure #328 — **Level 4 #3** `Bst<T>` binary search tree on a node arena: first arena-based ordered container. Backing struct `{ i64* keys, i32* left, i32* right, i64 root, i64 len, i64 capacity }`. Rotations / future balancing would swap i32 child indices rather than move nodes, preserving the affine single-owner contract. 7 builtins: `bst_new()`, `bst_insert(mut ref, x) -> bool` (true on new, false on duplicate), `bst_contains(ref, x) -> bool`, `bst_remove(mut ref, x) -> bool` (true if removed — in-order successor for two-children case), `bst_len(ref) -> i64`, `bst_min(ref) -> Option<i64>`, `bst_max(ref) -> Option<i64>`. Method sugar: `b.insert(x)` / `.contains(x)` / `.remove(x)` / `.len()` / `.min()` / `.max()`. v1 ships an unbalanced BST — AVL / red-black rotations are queued for a follow-up. Scope-exit Drop frees the three arrays. New `examples/bst.vani` insert/query/remove demo; 6 new lib tests. Closure #327 (Level 4 #6) Bloom filter shipped immediately before. Roadmap: **Level 4 is now 4/9 complete** — next, graphs + algorithms (#329) (BFS/DFS/Dijkstra unblocked by BinaryHeap, Kruskal unblocked by UnionFind), Trie (#330), skip list (#331), and AVL/RB balancing on the Bst arena (queued).)
**Test totals:** 1215 lib + 54 end-to-end + 11 vtables-phase3 + 2 user-drop-by-ref + 1 ssa-examples tests passing; the cross-backend parity runner covers all 83 examples under `examples/`. (Win32 LLVM dispatch adds 4 host-gated tests that fire on Windows hosts only — futex/WaitOnAddress, CreateThread for tasks, plus the CreateThread fan-out parallel-for tests in tree-LLVM and SSA-LLVM.)
