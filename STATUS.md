...
**Last updated:** 2026-05-30 (closure #392 — **`vec_max_by(ref xs, key) / vec_min_by(ref xs, key) -> Option<i64>`** with method sugar `xs.max_by(k) / xs.min_by(k)`. Returns the element with the largest / smallest key score (key fn has shape `fn(i64) -> i64`). Returns None for empty Vec. C: per-Vec helpers with a typedef'd `key_fn` pointer + running-best scan; gated on Option<i64> registry. LLVM: parallel define pair generated from `[("max_by", "sgt"), ("min_by", "slt")]` tuple loop — the only difference is the comparison predicate. Method sugar `.max_by(k)` / `.min_by(k)` registered alongside the other Vec methods. Both backends byte-identical: `vec_max_by([-7, 3, 5, -2], abs) = Some(-7)`. 2 new lib tests pin typecheck + method sugar. 1385 lib + 54 parity green. Closure #391 (option_and_then) shipped immediately before.)
