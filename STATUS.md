...
**Last updated:** 2026-05-30 (closure #370 — **`sort_desc(mut ref xs)` in-place descending sort**. New builtin that composes `sort()` + `reverse()` at the call site — no new runtime helper. Works on `Vec<i64>` and `[i64; N]`. Method sugar `xs.sort_desc()` registers in the table alongside `sort` / `sort_by`. C: statement-expression that calls both `intent_vec_int64_t__sort` and `intent_vec_int64_t__reverse` (plus the Array variants) and yields 0. LLVM: same shape — two consecutive `call i64 @intent_vec_<tag>__sort` and `@intent_vec_<tag>__reverse` instructions, then `ret 0`. Both backends byte-identical: `[5,2,8,1,9,3].sort_desc()` → `[9,8,5,3,2,1]`. 3 new lib tests pin free-function + method-sugar paths + by-value rejection. 1332 lib + 54 parity green. Closure #369 (str case conversion) shipped immediately before.)
