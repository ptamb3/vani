...
**Last updated:** 2026-05-29 (closure #354 — **Level 4 container `clear()` suite for Deque / BinaryHeap / BloomFilter / Bst / Trie / SkipList**: each gains `_clear(mut ref c) -> i64` returning prior len. Three flavors of reset:  (a) Deque/BinaryHeap/Bst free all backing buffers and zero everything (no head sentinel); (b) BloomFilter `memset`-zeros the bit array and resets `insert_count`, keeping the `num_bits`/`num_hashes` config; (c) Trie/SkipList keep buffer capacity and reset to the single-root/single-head-sentinel state (the next insert walks a valid root/head). LLVM declares an extern `@memset(i8*, i32, i64)` for the BloomFilter helper. Method sugar `.clear()` across all six. Both backends byte-identical. 2 new lib tests pin typecheck and helper-name emission for all six containers. 1285 lib + 54 parity green across 90 examples. Closure #353 (HashSet/HashMap/BTreeSet/BTreeMap clear) shipped immediately before. This completes the 2-closure container-clear lump; every container in the language now has a clear() method for buffer reuse.)
