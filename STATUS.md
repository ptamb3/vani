...
**Last updated:** 2026-05-29 (closure #332 — **AVL balancing on the existing `Bst<T>` arena** (Level 4 #3 extension, no new type). Adds per-node `u8* heights` array as a 7th struct field (kept at the end so all existing LLVM GEP indices for fields 0..5 stay valid). Insert and remove now walk the search path on stack-allocated `[64 x i64]` + `[64 x i8]` buffers (depth bound well above the AVL height of any tree that fits in i32 child indices), then walk back up applying the four rotation cases (LL / LR / RR / RL) via shared rotate_left / rotate_right / rebalance helpers. Rotations swap i32 child indices in place — no node moves, so the affine ownership story is unchanged. Sorted-input adversarial workloads (1..15 ascending, 15..1 descending, zig-zag) now run in O(log n) time per op; previously they degenerated to O(n) under the v1 unbalanced BST. New `examples/bst_avl.vani` exercises the adversarial patterns + mid-tree removes that trigger the two-children + successor-walk + rebalance path; 1 new lib test pins the runtime + helper names + new field layout. Closure #331 (SkipList) shipped immediately before. Roadmap: **Level 4 of the data-structures roadmap is now fully closed under v1** (#324 vec_chain, #325 UnionFind, #326 BinaryHeap, #327 Bloom, #328 Bst, #329 Graph, #330 Trie, #331 SkipList, #332 AVL-on-Bst). Remaining queued items are extensions: graph algorithms (A* / topo / Kruskal / Prim, CSR adjacency cache), radix-compressed Trie, SkipList remove + tail tracker, and the post-Level-4 closure-as-value type for combinator args.)
**Test totals:** 1234 lib + 54 end-to-end + 11 vtables-phase3 + 2 user-drop-by-ref + 1 ssa-examples tests passing; the cross-backend parity runner covers all 87 examples under `examples/`. (Win32 LLVM dispatch adds 4 host-gated tests that fire on Windows hosts only — futex/WaitOnAddress, CreateThread for tasks, plus the CreateThread fan-out parallel-for tests in tree-LLVM and SSA-LLVM.)
