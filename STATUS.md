...
**Last updated:** 2026-05-31 (closure #426 — **`str_byte_at(s, i) -> i64`** + **`str_len_bytes(s) -> i64`** byte-level access on `Str`. `str_byte_at(s, i)` returns the i-th byte of s (as i64 in `[0, 255]`), `str_len_bytes(s)` returns `strlen(s)`. Useful for byte-level string processing where the existing `str_chars` / `substring` machinery is too heavy. No bounds checks — caller's responsibility to ensure `0 <= i < strlen(s)` (matches the safety contract of pointer arithmetic). C: `str_byte_at` is `((int64_t)(unsigned char)(s[i]))` direct array index; `str_len_bytes` wraps libc `strlen()`. LLVM: `str_byte_at` emits `getelementptr i8` + `load i8` + `zext i8 to i64`; `str_len_bytes` calls the already-declared `@strlen`. Both backends byte-identical: for `s = "Hello, World!"`, `byte_at(0) = 72` ('H'), `byte_at(4) = 111` ('o'), `byte_at(7) = 87` ('W'), `byte_at(12) = 33` ('!'); `len_bytes("") = 0`, `len_bytes("abc") = 3`. 2 new lib tests. 1440 lib + 54 parity green. Closure #425 (is_prime) shipped immediately before.)
