...
**Last updated:** 2026-05-31 (closure #418 — **`f64_next_up(x) -> f64`** + **`f64_next_down(x) -> f64`** next/prev representable double. `f64_next_up(x)` returns the smallest representable double strictly greater than `x`; `f64_next_down(x)` returns the largest representable double strictly less than `x`. Useful for interval arithmetic, ULP measurement, and floating-point algorithm boundary handling. C: routed through libm's `nextafter(x, INFINITY)` and `nextafter(x, -INFINITY)`. LLVM: declares `@nextafter(double, double)` and calls it with `0x7FF0000000000000` (+Inf) or `0xFFF0000000000000` (-Inf) as the second argument. Both backends byte-identical: `next_up(1.0) - 1.0 = 2.22045e-16` (= ULP = 2^-52 = epsilon), `1.0 - next_down(1.0) = 1.11022e-16` (= 2^-53, half-ULP below 1.0 because exponent steps), `next_up(0) = +smallest_subnormal = 4.94066e-324`, `next_down(0) = -smallest_subnormal = -4.94066e-324`, `next_up(max_finite) = +inf`, `next_down(-max_finite) = -inf`. 2 new lib tests (typecheck + `@nextafter` declare + +/-Inf bit-pattern emission). 1425 lib + 54 parity green. Closure #417 (IEEE classification predicates) shipped immediately before.)
