...
**Last updated:** 2026-05-31 (closure #430 — **`f64_step(edge, x) -> f64`** + **`f64_smoothstep(edge0, edge1, x) -> f64`** GLSL/HLSL-style step functions. `step(edge, x)` returns `0.0` if `x < edge`, else `1.0` — the hard threshold. `smoothstep(edge0, edge1, x)` does smooth Hermite interpolation: `t = clamp((x - edge0)/(edge1 - edge0), 0, 1); return t*t*(3 - 2*t)`. Saturates outside `[edge0, edge1]` to 0 / 1. Useful for shader-like animation curves, easing functions, anti-aliasing alpha computation. C: inline GNU statement-expressions. LLVM: inline IR — `step` uses `fcmp olt` + `select`; `smoothstep` uses `fsub`/`fdiv` for normalization, two `select` for clamp, then `fmul` chain for the cubic polynomial. Both backends byte-identical: `step(0.5, 0.3) = 0`, `step(0.5, 0.5) = 1`; `smoothstep(0, 1, -0.5) = 0`, `smoothstep(0, 1, 0) = 0`, `smoothstep(0, 1, 0.25) = 0.15625`, `smoothstep(0, 1, 0.5) = 0.5`, `smoothstep(0, 1, 0.75) = 0.84375`, `smoothstep(0, 1, 1.0) = 1`, `smoothstep(10, 20, 15) = 0.5` (proper normalization). 1 new lib test. 1446 lib + 54 parity green. Closure #429 (sigmoid/softsign) shipped immediately before.)
