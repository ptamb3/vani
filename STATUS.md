...
**Last updated:** 2026-05-29 (closure #353 — **Container `clear()` suite for HashSet / HashMap / BTreeSet / BTreeMap**: each gains `_clear(mut ref c) -> i64` returning prior len. Frees the backing buffer(s), zeros all struct fields (len / capacity / tombstones for hash-family), leaves the container reusable for fresh inserts. Method sugar `.clear()` across all four. Both backends byte-identical. 3 new lib tests pin typecheck, by-ref rejection, and helper-name emission for all four containers. 1283 lib + 54 parity green across 90 examples. Closure #352 (btree min/max) shipped immediately before. Lumped as the first of two container-clear closures — #354 adds clear to BinaryHeap / Bst / SkipList / Trie / Deque / BloomFilter next.)
