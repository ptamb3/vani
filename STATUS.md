...
**Last updated:** 2026-05-30 (closure #368 — **`str_repeat(s: Str, n: i64) -> OwnedStr`**. Concatenates `s` with itself `n` times in a freshly-malloc'd buffer. Edge cases all collapse to a fresh empty string: negative `n`, zero `n`, NULL `s`, empty `s`. C: dedicated `intent_str_repeat` helper with `SIZE_MAX` overflow guard before the multiply, then `memcpy` in a for-loop. LLVM: dedicated `define i8* @intent_str_repeat(i8*, i64)` with phi-driven for-loop blocks (`sr_loop_head` / `sr_loop_body` / `sr_finish` / `sr_empty`) — uses `getelementptr` for per-iteration destination computation. Both backends byte-identical: `str_repeat("ab", 3) = "ababab"`, all edge cases produce empty. 3 new lib tests pin typecheck + helper-name emission + arity-2 requirement. 1326 lib + 54 parity green. Closure #367 (math constants) shipped immediately before.)
