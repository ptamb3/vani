...
**Last updated:** 2026-05-30 (closure #359 — **`f64_to_str(x: f64) -> OwnedStr`**: extends the parse/stringify round-trip to floating-point alongside the matching `i64_to_str` from #358. Produces a compact decimal representation of `x` as a freshly malloc'd OwnedStr; uses `snprintf(..., "%g", x)` with a 32-byte scratch buffer (clamped to 31 on the unlikely overrun path, then NUL-terminated). C emits the new `intent_f64_to_str` helper alongside the existing `intent_i64_to_str` in the same emitter function (both stay `INTENT_UNUSED`-gated). LLVM mirrors via a new `define i8* @intent_f64_to_str(double %x)` and the pre-existing `@.fmt.g` format global + `@snprintf` extern. Both backends byte-identical: `3.14 / 0 / -2.5 / 1e+06`, plus `"pi=" + f64_to_str(3.14)` concat works the same way as the i64 variant. 3 new lib tests pin typecheck + both-backend helper emission + the f64-literal coercion path. 1297 lib + 54 parity green. Closure #358 (i64_to_str) shipped immediately before.)
