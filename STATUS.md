...
**Last updated:** 2026-05-31 (closure #440 — **`f64_sinc(x) -> f64`** + **`f64_safe_div(a, b, default) -> f64`** signal processing + safe division. `sinc(x) = sin(x)/x` with `sinc(0) = 1` (the removable singularity at zero handled explicitly via `fcmp oeq` + `select`). Used in interpolation kernels, DSP, antialiasing. `safe_div(a, b, default) = a/b` if `b != 0`, else `default` — provides a defensive division without panicking on zero divisor; useful for guards in math pipelines. C: inline GNU statement-expressions with ternary on the singular case. LLVM: `sinc` uses `fcmp oeq 0` + `call @sin` + `fdiv` + `select`; `safe_div` uses `fcmp oeq 0` + `fdiv` + `select`. Both backends byte-identical: `sinc(0) = 1`, `sinc(π) ≈ 3.89817e-17` (FP noise around the true zero), `sinc(2π) ≈ -3.89817e-17`, `sinc(1) = 0.841471` (= sin(1)), `sinc(-1) = 0.841471` (sinc is even); `safe_div(10, 2, -999) = 5`, `safe_div(10, 0, -999) = -999` (default used), `safe_div(-5, 2.5, 0) = -2`, `safe_div(0, 0, 42) = 42`. 1 new lib test. 1458 lib + 54 parity green. Closure #439 (swish/logit) shipped immediately before.)
