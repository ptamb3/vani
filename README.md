...
| Iter combinators | Eager `vec_map(ref xs, f) -> Vec<i64>` + `vec_filter(ref xs, p) -> Vec<i64>` + `vec_fold(ref xs, init, g) -> i64` on Vec<i64> (closures #309 + #310); slicing `vec_take(ref xs, n)` / `vec_drop(ref xs, n)` (closure #313); fused `vec_map_fold(ref xs, init, f, g) -> i64` single-pass (closure #316). Pair with anon fns or top-level fn-refs | ✅ AFFINE |
...
| **3 — Closures + iterators** | ✅ Anonymous fn expressions w/o captures (#308) · ✅ Eager `vec_map` / `vec_fold` / `vec_filter` on Vec<i64> via fn-ptr args (#309 + #310) · ✅ Method-call sugar across Vec + affine containers (#311 + #312) · ✅ `vec_take` / `vec_drop` + uniform `xs.len()` (#313) · ✅ Closures w/ captured state — capture-by-value Copy types, callable in-fn only (#314). ⏳ Non-Copy captures; capture-by-ref second-class closures; passing closures as fn-ptr args; loop fusion for combinator chains; `.collect()` / `vec_zip` | ✅ / ⚠️ AFFINE-TENSION |
