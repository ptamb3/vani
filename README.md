...
| Graph | `Graph` weighted directed graph; per-edge parallel `edge_src` / `edge_dst` (i32) + `edge_weight` (i64) arrays; 10 builtins: `new` / `add_edge` / `num_nodes` / `num_edges` / `bfs_reach` / `dfs_reach` / `dijkstra` / `has_cycle` (Kahn) / `mst_kruskal` (sort + Union-Find, undirected interpretation) / `mst_prim` (O(V²), undirected) + method sugar; `dijkstra` / `mst_*` return `Option<i64>`; v1 i64 weights, i32 nodes (closure #329 + algorithm extensions in **#333**, **Level 4 #5**) | ✅ AFFINE |
...
| SkipList | `SkipList<T>` probabilistic ordered set on a node arena; MAX_LEVEL=8 (v1); flat capacity × 8 i32 `forward` index array + per-node `node_levels`; geometric level distribution from a stored LCG seed; 6 builtins (`new` / `insert` / `contains` / `len` / `min` / `max`) + method sugar; `min` / `max` return `Option<i64>` (closure #331, **Level 4 #7**) | ✅ AFFINE |
