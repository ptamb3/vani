...
| **2 — Generic containers** (deps: Level 1, generic decls #281) | ✅ BinaryHeap-on-Vec (#302) · ✅ `Deque<i64>` ring buffer (#303) · ✅ `HashSet<i64>` open-addressing (#304). ⏳ `HashMap<K, V>` (⚠️ — `get -> Option<ref V>`, `insert` consumes, `remove` moves), `BTreeSet<T>` (✅ Copy / ⚠️ owning), `BTreeMap<K, V>` (⚠️ same contract), dedicated `BinaryHeap<T>` wrapper type (deferred ergonomic layer) | ✅ / ⚠️ AFFINE-TENSION |
