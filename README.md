...
| Method-call sugar | `xs.map(f)` / `xs.filter(p)` / `xs.fold(init, g)` / `xs.sort_by(cmp)` / `xs.sort()` on `Vec<T>` receivers desugar to the builtins (closure #311) | ✅ AFFINE |
...
| **3 — Closures + iterators** | ✅ Anonymous fn expressions w/o captures (#308, lambda-lifted to `__anon_fn_<N>`) · ✅ Eager `vec_map` / `vec_fold` / `vec_filter` on Vec<i64> via fn-ptr args (#309 + #310) · ✅ Method-call sugar `xs.map(f)` / `xs.filter(p)` / `xs.fold(init, g)` / `xs.sort_by(cmp)` (#311). ⏳ Closures w/ captured state (⚠️ capture-by-value moves; capture-by-ref produces a second-class closure); loop fusion for combinator chains; `.collect()` / `vec_zip` / `vec_take`; `sort_by` / `find_by` lifted to closure | ✅ / ⚠️ AFFINE-TENSION |
