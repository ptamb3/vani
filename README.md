...
| Trie | `Trie` prefix tree on a node arena; flat 26 × num_nodes i32 `children` + per-node `is_end` byte; 7 builtins (`new` / `insert` / `contains` / `starts_with` / `delete` / `len` / `node_count`) + method sugar; v1 a-z alphabet — operations short-circuit to false on any non-a-z input character; delete tombstones the is_end bit (no arena compaction) (closure #330 + delete added in **#340**, **Level 4 #4**) | ✅ AFFINE |
| SkipList | `SkipList<T>` probabilistic ordered set on a node arena; MAX_LEVEL=8 (v1); flat capacity × 8 i32 `forward` index array + per-node `node_levels`; geometric level distribution from a stored LCG seed; 7 builtins (`new` / `insert` / `contains` / `remove` / `len` / `min` / `max`) + method sugar; `min` / `max` return `Option<i64>`; remove tombstones the slot (no compaction) (closure #331 + remove added in **#339**, **Level 4 #7**) | ✅ AFFINE |
