...
| Iter combinators | Eager `vec_map(ref xs, f) -> Vec<i64>` + `vec_fold(ref xs, init, g) -> i64` on Vec<i64> (closure #309). Pair with anon fns or top-level fn-refs | ✅ AFFINE |
...
| **3 — Closures + iterators** | ✅ Anonymous fn expressions w/o captures (#308, lambda-lifted to `__anon_fn_<N>`) · ✅ Eager `vec_map` / `vec_fold` on Vec<i64> via fn-ptr args (#309). ⏳ Closures w/ captured state (⚠️ capture-by-value moves; capture-by-ref produces a second-class closure); loop fusion for `vec_map / vec_fold` chains; `vec_filter` / `.collect()` / `vec_zip`; `sort_by` / `find_by` lifted to closure | ✅ / ⚠️ AFFINE-TENSION |
