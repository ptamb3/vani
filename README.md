...
| String ops | `str_contains` / `str_starts_with` / `str_ends_with` -> bool, `parse_int` / `parse_float` -> `Option<i64>` / `Option<f64>`, `str_trim` -> `OwnedStr` (heap-allocating, strips ASCII whitespace), `str_replace(s, from, to)` -> `OwnedStr` (two-pass non-overlapping substring replace) (closure #298 + `str_trim` in **#348** + `str_replace` in **#349**) | ✅ AFFINE |
