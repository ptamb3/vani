...
| Trie | `Trie` prefix tree on a node arena; flat 26 × num_nodes i32 `children` + per-node `is_end` byte + `free_head`/`free_count` freelist; 7 builtins (`new` / `insert` / `contains` / `starts_with` / `delete` / `len` / `node_count`) + method sugar; v1 a-z alphabet — operations short-circuit to false on any non-a-z input character; delete + arena compaction walks back up the path freeing dead-end nodes; recycled slots reused by future `insert`s; `node_count` reflects live nodes only (closure #330 + delete in **#340** + arena compaction in **#344**, **Level 4 #4**) | ✅ AFFINE |
