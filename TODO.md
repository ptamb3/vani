...
## ⏳ Resume here (paused 2026-05-29, after closure #349 — **`str_replace(s: Str, from: Str, to: Str) -> OwnedStr`**. Second heap-allocating string builtin after #348's `str_trim`. Replaces every non-overlapping occurrence of `from` in `s` with `to`. Two-pass implementation: walk via `strstr` to count matches and size the output buffer exactly, then walk again copying spans + replacements. Edge cases handled: empty `from` is a no-op (otherwise the search loop diverges — return a fresh dup); empty `to` deletes every occurrence; no-match returns a fresh dup; UTF-8 byte sequences round-trip. C helper uses libc `strstr` + `memcpy`; LLVM mirrors with two scan loops and `ptrtoint` span arithmetic. Extended `examples/string_ops.vani` covers single match, growing replace, shrinking replace, delete, empty-from no-op, no-match. Both backends byte-identical. 3 new lib tests pin typecheck shape (3 args → OwnedStr), arity diagnostic, helper-name emission. 1269 lib + 54 parity green across 90 examples. Closure #348 (`str_trim`) shipped immediately before. Next focal area sequence: (#350+) **`str_split(s, delim) -> Vec<OwnedStr>`** — completes the heap-allocating string trio. Needs per-slot OwnedStr emission into a `Vec<OwnedStr>` (the Vec runtime already supports the element type). After that: SipHash (adversarial-resistant); `Hash`/`Ord` interface for user struct keys. Further out: **Closure-as-value type richer support** — capture-by-ref, non-Copy captures, `.collect()`, lazy iterators, non-i64 element types in combinators, tuple-element `vec_zip`. Future Trie work: sparse per-node children via `Vec<(u8, u32)>` for memory efficiency, ordered enumeration. Deferred: non-Copy V (Option<ref V> AFFINE-TENSION shift), wider K/V widths, expression-body anon-fn shorthand, generic anon fns, async, Kosh.)
