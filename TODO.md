...
## ⏳ Resume here (paused 2026-05-28, after closure #319 — Level 3 auto-fusion phase 2: extends the closure #318 peephole pass to all four producer→consumer combinations (`map+fold`, `filter+fold`, `map+filter`, `map_filter+fold`). The 3-stage `map → filter → fold` chain fuses iteratively into a single `vec_map_filter_fold` call — zero intermediate Vec allocations. Refactored via `ProducerKind`/`ConsumerKind` enums. 3 new lib tests. Next focal area sequence: (#320) `vec_zip` (needs Vec<(i64, i64)> tuple element type) — adds the first tuple-element Vec to the language, opening up Vec<tuple> patterns broadly; `.collect()` (once lazy iterators land); non-i64 element types in combinators (`Vec<f64>` mapper variants); type-changing map (`Vec<i64> -> Vec<Str>`); method-call sugar for Vec mutators (push / pop / reverse / dedup); chains across non-adjacent statements. Closure follow-ups: non-Copy captures with move semantics; capture-by-ref second-class closures; passing closures as fn-ptr args (needs env+ptr struct value type); closure-name reuse across sibling scopes. After Level 3 closes: Level 4 arena-based BST/AVL/B-tree/Trie/graphs + algorithms. Deferred: hashmap_remove, hashset_remove, non-Copy V (Option<ref V> AFFINE-TENSION shift), wider K/V widths, btreeset/btreemap range queries + iteration via closures, expression-body anon-fn shorthand, generic anon fns, SipHash, hash_f64, Hash/Ord interface for user structs, heap-allocating str_split / str_trim / str_replace, dedicated BinaryHeap<T> wrapper type, async, Kosh.)
