...
## ⏳ Resume here (paused 2026-05-29, after closure #350 — **`str_split(s: Str, delim: Str) -> Vec<OwnedStr>`**. Third and final heap-allocating string builtin; joins `str_trim` (#348) and `str_replace` (#349) to complete the trio. Returns a fresh `Vec<OwnedStr>` whose elements are non-overlapping spans of `s` between occurrences of `delim`. Each element is a freshly malloc'd `char*` so the Vec's per-element Drop chain frees every span on scope exit. Single-pass scan via libc `strstr` with inline push (no per-type-helper dependency) and capacity-doubling growth starting at 4. Edge cases: empty `delim` returns a single-element vec containing `dup(s)`; empty `s` returns a single-element vec with an empty OwnedStr; consecutive delims yield empty middle elements; leading/trailing delims yield empty bracket elements; no-match returns a single-element vec with `dup(s)`. Helper emission is gated on actual usage via a new `program_uses_str_split` walker, since the signature references `%intent_vec_i8p`/`intent_vec_owned_str` which are themselves element-walker-gated. LLVM emits the helper after the Vec typedef is in scope. Extended `examples/string_ops.vani` covers basic split, multi-char delim, consecutive delims, no-match. Both backends byte-identical. 3 new lib tests pin typecheck shape, helper-name emission, unused-gating. 1272 lib + 54 parity green across 90 examples. Closure #349 (`str_replace`) shipped immediately before. Next focal area sequence: (#351+) **SipHash** — adversarial-resistant hash to complement FNV-1a (`hash_i64` / `hash_f64` / `hash_str` / `hash_combine`); useful for HashSet/HashMap on attacker-controlled keys. After that: **`Hash`/`Ord` interface for user struct keys** — would let users put their own struct types into HashSet/HashMap/BTreeSet/BTreeMap by implementing the trait. Further out: **Closure-as-value type richer support** — capture-by-ref, non-Copy captures, `.collect()`, lazy iterators, non-i64 element types in combinators, tuple-element `vec_zip`. Future Trie work: sparse per-node children via `Vec<(u8, u32)>` for memory efficiency, ordered enumeration. Deferred: non-Copy V (Option<ref V> AFFINE-TENSION shift), wider K/V widths, expression-body anon-fn shorthand, generic anon fns, async, Kosh.)
