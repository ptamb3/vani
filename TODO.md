...
## ⏳ Resume here (paused 2026-05-29, after closure #346 — **BTreeSet / BTreeMap range queries**. Three new builtins on the existing sorted-Vec backings: `btreeset_range(ref s, lo, hi, mut ref out: Vec<i64>) -> i64`, `btreemap_range_keys(ref m, lo, hi, mut ref out) -> i64`, and `btreemap_range_values(ref m, lo, hi, mut ref out) -> i64`. Each uses the existing `lower_bound` helper to seek to `lo` then walks forward until `keys[i] > hi`, appending matching entries to `out` (capacity-doubling growth, mirroring `graph_topo_sort`). Method sugar: `s.range(lo, hi, mut ref out)`, `m.range_keys(...)`, `m.range_values(...)`. Helpers gated on `intent_vec_int64_t` being available (extended `program_uses_graph_vec_builtin` walker). Extended `examples/btreeset.vani` + `examples/btreemap.vani` exercise the typical case, empty range, and `lo>hi`. Both backends byte-identical. 5 new lib tests pin typecheck (3 args after receiver, `mut ref Vec<i64>` enforcement on out, helper-name emission). 1260 lib + 54 parity green across 90 examples. Closure #345 (Trie u8 alphabet) shipped immediately before. Next focal area sequence: (#347+) **Closure-as-value type richer support** — capture-by-ref, non-Copy captures, `.collect()`, lazy iterators, non-i64 element types in combinators, tuple-element `vec_zip`. Future Trie work: sparse per-node children via `Vec<(u8, u32)>` for memory efficiency, ordered enumeration. Deferred: non-Copy V (Option<ref V> AFFINE-TENSION shift), wider K/V widths, expression-body anon-fn shorthand, generic anon fns, SipHash, hash_f64, Hash/Ord interface for user structs, heap-allocating str_split / str_trim / str_replace, async, Kosh.)
