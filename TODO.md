...
## ⏳ Resume here (paused 2026-05-29, after closure #345 — **Trie alphabet generalization (a-z → full u8)**. The per-node child stride changed from 26 to 256, dropping the lowercase-a-z restriction. Every nonzero u8 byte is now a valid character (mixed-case, punctuation, digits, all UTF-8 byte sequences). `intent_trie_valid_str` simplified to a non-NULL check. Per-node memory grew from 104 bytes (26 × i32) to 1024 bytes (256 × i32); the LLVM realloc-size constant moved from `mul %cap_new, 104` to `mul %cap_new, 1024`. `sext i8 → i64` for character bytes became `zext i8 → i64` so bytes 128-255 stay positive in indexing. Extended `examples/trie.vani` exercises mixed-case + punctuation insertion (`"Hi!"` is no longer "invalid"). Both backends byte-identical. 1 new lib test pins the 256-wide stride. 1255 lib + 54 parity green across 90 examples. Closure #344 (Trie arena compaction) shipped immediately before. Next focal area sequence: (#346+) **Closure-as-value type** — unblocks A* path reconstruction, real Prim/Kruskal weight comparators, sort_by with closures, and the broader iterator-combinator work. After Level 4 extensions: tuple-element Vec for `vec_zip`; non-Copy captures; `.collect()` + lazy iterators; non-i64 element types in combinators. Future Trie work: sparse per-node children via `Vec<(u8, u32)>` for memory efficiency, ordered enumeration. Deferred: non-Copy V (Option<ref V> AFFINE-TENSION shift), wider K/V widths, btreeset/btreemap range queries, expression-body anon-fn shorthand, generic anon fns, SipHash, hash_f64, Hash/Ord interface for user structs, heap-allocating str_split / str_trim / str_replace, async, Kosh.)
