...
## ⏳ Resume here (paused 2026-05-28, after closure #312 — Level 3 surface ergonomics: method-call sugar extended to the five affine containers (HashMap / HashSet / BTreeMap / BTreeSet / Deque). `m.get(k)` / `s.insert(v)` / `d.push_back(v)` / `m.contains_key(k)` / `m.remove(k)` / `m.len()` etc. desugar to the existing `<container>_<method>` builtins. Parser extended to accept the `len` keyword as a method name after `.`. **Bug fix bundled:** SSA-LLVM `emit` now clears `LLVM_ENUM_PAYLOAD_REGISTRY` + the variant-payload / tag registries at the top — a stale leak from a previous tree-LLVM compile in the same process was causing `intent_vec_i64__heap_peek` to emit `%Enum_Option__i64` references without the typedef when programs were batched (e.g. via `intentc test <many files>`). 6 new lib tests. Next focal area sequence: (#313) `vec_take` / `vec_zip` / `.collect()` — independent of closures, eager Vec slicing combinators. (#314, BIG) **closures with captured environment** (Level 3 #1 phase 2) — capture-by-value moves the binding (affine); capture-by-ref produces second-class closures. Unblocks async and smarter combinators. (#315) Loop fusion at monomorphization time so chained combinators don't allocate intermediate Vecs. Then non-i64 element types; type-changing map (`Vec<i64> -> Vec<Str>`); method-call sugar for Vec mutators (push / pop / reverse / dedup); `.len()` uniform method on Vec. After Level 3 closes: Level 4 arena-based BST/AVL/B-tree/Trie/graphs + algorithms. Deferred: hashmap_remove, hashset_remove, non-Copy V (Option<ref V> AFFINE-TENSION shift), wider K/V widths, btreeset/btreemap range queries + iteration via closures, expression-body anon-fn shorthand, generic anon fns, SipHash, hash_f64, Hash/Ord interface for user structs, heap-allocating str_split / str_trim / str_replace, dedicated BinaryHeap<T> wrapper type, async, Kosh.)
