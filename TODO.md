...
## ⏳ Resume here (paused 2026-05-29, after closure #347 — **`hash_f64(x: f64) -> u64`**. Completes the hash family started at closure #301. FNV-1a over the IEEE-754 bit pattern: bitcast the `double` to a `u64`, then run the same 8-byte FNV-1a loop as `hash_i64`. Folds `-0.0` to `+0.0` before bitcast so the two values (which compare equal under `==`) share a hash. NaN payloads are NOT normalized — per IEEE-754, `NaN != NaN`. C uses `memcpy(&u, &x, 8)` (no aliasing UB); LLVM uses `bitcast double to i64` and tail-calls `@intent_hash_i64`. Hash-helper gate (LLVM `program_uses_hash`) extended to include `hash_f64`. Extended `examples/hash.vani` exercises determinism, distinct inputs, and the `-0.0`/`+0.0` fold. 3 new lib tests pin typecheck + helper-name emission. 1263 lib + 54 parity green across 90 examples. Closure #346 (BTreeSet/BTreeMap range queries) shipped immediately before. Next focal area sequence: (#348+) **Heap-allocating string operations** — `str_trim` / `str_split` / `str_replace`. Each returns OwnedStr (or `Vec<OwnedStr>` for `split`), with the existing OwnedStr drop machinery handling cleanup. After that: SipHash (adversarial-resistant); `Hash`/`Ord` interface for user struct keys. Further out: **Closure-as-value type richer support** — capture-by-ref, non-Copy captures, `.collect()`, lazy iterators, non-i64 element types in combinators, tuple-element `vec_zip`. Future Trie work: sparse per-node children via `Vec<(u8, u32)>` for memory efficiency, ordered enumeration. Deferred: non-Copy V (Option<ref V> AFFINE-TENSION shift), wider K/V widths, expression-body anon-fn shorthand, generic anon fns, async, Kosh.)
