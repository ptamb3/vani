...
## ⏳ Resume here (paused 2026-05-29, after closure #348 — **`str_trim(s: Str) -> OwnedStr`**. First heap-allocating member of the string builtin family (the previous five — `str_contains` / `str_starts_with` / `str_ends_with` / `parse_int` / `parse_float` — were all allocation-free). Strips leading and trailing ASCII whitespace (space, `\t`, `\n`, `\v`, `\f`, `\r` — the standard `isspace()` set) and returns a fresh OwnedStr handle. Output is always heap-owned so the scope-exit Drop is well-defined; even no-op trims and empty inputs hand back a freshly-malloc'd buffer. Internal whitespace and UTF-8 byte sequences preserved. C uses a two-pointer scan + memcpy; LLVM uses a six-way OR of the whitespace bytes plus forward/backward scan. Both helpers emitted unconditionally (matching the existing `intent_str_concat` convention). Extended `examples/string_ops.vani` exercises mixed whitespace, internal-whitespace preservation, no-op trim, and all-whitespace input. Both backends byte-identical. 3 new lib tests pin typecheck, concat-compatibility (`str_trim(a) + str_trim(b)`), and helper-name emission. 1266 lib + 54 parity green across 90 examples. Closure #347 (`hash_f64`) shipped immediately before. Next focal area sequence: (#349+) **`str_split(s, delim) -> Vec<OwnedStr>`** and **`str_replace(s, from, to) -> OwnedStr`** — completes the heap-allocating string trio. After that: SipHash (adversarial-resistant); `Hash`/`Ord` interface for user struct keys. Further out: **Closure-as-value type richer support** — capture-by-ref, non-Copy captures, `.collect()`, lazy iterators, non-i64 element types in combinators, tuple-element `vec_zip`. Future Trie work: sparse per-node children via `Vec<(u8, u32)>` for memory efficiency, ordered enumeration. Deferred: non-Copy V (Option<ref V> AFFINE-TENSION shift), wider K/V widths, expression-body anon-fn shorthand, generic anon fns, async, Kosh.)
