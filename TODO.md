...
## ⏳ Resume here (paused 2026-05-28, after closure #320 — Level 3 ergonomics polish: method-call sugar extended to all in-place Vec mutators (`xs.push(v)`, `xs.pop()`, `xs.reverse()`, `xs.dedup()`, `xs.swap_remove(i)`, `xs.insert(i, v)`, `xs.clear()`) and read-only search builtins (`xs.find(v)`, `xs.contains(v)`, `xs.binary_search(v)`). Same `Var` receiver restriction as #311. Pure checker desugar. 3 new lib tests. Next focal area sequence: (#321) `vec_zip` — needs `Vec<(i64, i64)>` tuple-element-Vec; this opens up Vec<tuple> patterns broadly and is a real new feature; (#322) `.collect()` and non-i64 element types in combinators (e.g. `Vec<f64>` mapper variants); type-changing map (`Vec<i64> -> Vec<Str>`); auto-fusion across non-adjacent statements; method sugar for Array-typed receivers. Closure follow-ups: non-Copy captures with move semantics; capture-by-ref second-class closures; passing closures as fn-ptr args (needs env+ptr struct value type); closure-name reuse across sibling scopes. After Level 3 closes: Level 4 arena-based BST/AVL/B-tree/Trie/graphs + algorithms. Deferred: hashmap_remove, hashset_remove, non-Copy V (Option<ref V> AFFINE-TENSION shift), wider K/V widths, btreeset/btreemap range queries + iteration via closures, expression-body anon-fn shorthand, generic anon fns, SipHash, hash_f64, Hash/Ord interface for user structs, heap-allocating str_split / str_trim / str_replace, dedicated BinaryHeap<T> wrapper type, async, Kosh.)
