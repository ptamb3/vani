...
fn check_math_builtin(
    ...
    let want_args = match name {
        ...
        | "i64_wrap" | "f64_wrap" | "i64_safe_div" => 3,
        "f64_pi" | "f64_e" | "f64_inf" | "f64_nan"
...
fn check_str_builtin(
    ...
    // Closure #454: str_count_ascii_digits(s: Str) -> i64.
    // Counts the number of decimal-digit bytes ('0'..'9') in s.
    if name == "str_count_ascii_digits" {
        if args.len() != 1 {
            diagnostics.push(Diagnostic::new(
                span,
                format!("str_count_ascii_digits() expects 1 argument, got {}", args.len()),
            ));
            return CheckedExpr::fallback(Type::I64, span);
        }
        let s_raw = check_expr(&args[0], env, signatures, diagnostics);
        let s = coerce_checked(
            s_raw, &Type::Str, args[0].span,
            "str_count_ascii_digits argument", diagnostics,
        );
        return CheckedExpr::new(
            TypedExprKind::Call {
                name: "str_count_ascii_digits".to_string(),
                name_span: span,
                args: vec![s.expr],
            },
            Type::I64,
