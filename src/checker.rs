...
fn check_str_builtin(
    ...
    // Closure #466: str_first_byte(s) -> Option<i64>.
    // Returns Some(byte) for non-empty s, None for empty.
    if name == "str_first_byte" {
        if args.len() != 1 {
            diagnostics.push(Diagnostic::new(
                span,
                format!("str_first_byte() expects 1 argument, got {}", args.len()),
            ));
        ...
        let s = coerce_checked(
            s_raw, &Type::Str, args[0].span,
            "str_first_byte argument", diagnostics,
        );
        return CheckedExpr::new(
            TypedExprKind::Call {
                name: "str_first_byte".to_string(),
                name_span: span,
                args: vec![s.expr],
            },
            Type::Enum(mangle_generic_decl("Option", &[Type::I64])),
