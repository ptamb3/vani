    ...
    fn walk_expr_for_search_builtins(
        ...
        if let ExprKind::Call { name, args, .. } = &expr.kind {
            ...
            let needed_arg: Option<Type> = match name.as_str() {
                ...
                | "str_index_of_byte" | "vec_position"
...
fn check_call(
    ...
    match name {
        ...
        | "str_ends_with_byte"
        // Closure #441: byte count + Closure #442: byte index.
        | "str_byte_count"
        | "str_index_of_byte" => {
...
fn check_str_builtin(
    ...
    // Closure #442: str_index_of_byte(s, b) -> Option<i64>.
    // Returns Some(i) where i is the first index of byte b in s,
    // or None if not found.
    if name == "str_index_of_byte" {
        if args.len() != 2 {
            diagnostics.push(Diagnostic::new(
                span,
                format!("str_index_of_byte() expects 2 arguments, got {}", args.len()),
            ));
        ...
        let s = coerce_checked(
            s_raw, &Type::Str, args[0].span,
            "str_index_of_byte `s` argument", diagnostics,
        );
        let b_raw = check_expr(&args[1], env, signatures, diagnostics);
        let b = coerce_checked(
            b_raw, &Type::I64, args[1].span,
            "str_index_of_byte `b` argument", diagnostics,
        );
        return CheckedExpr::new(
            TypedExprKind::Call {
                name: "str_index_of_byte".to_string(),
                name_span: span,
