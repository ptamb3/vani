...
fn check_str_builtin(
    ...
    // Closures #454 + #455: str_count_ascii_digits / _alpha — (Str) -> i64.
    if matches!(name, "str_count_ascii_digits" | "str_count_ascii_alpha") {
        if args.len() != 1 {
            diagnostics.push(Diagnostic::new(
                span,
