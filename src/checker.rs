...
fn check_vec_utility_builtin(
    ...
    let want_args = match name {
        "vec_reverse_copy" | "vec_unique" | "vec_iota"
        | "vec_first" | "vec_last" | "vec_running_sum" => 1,
        _ => 2,
    };
    if args.len() != want_args {
        diagnostics.push(Diagnostic::new(
    ...
    if matches!(name, "vec_reverse_copy" | "vec_unique" | "vec_running_sum") {
        let ok = matches!(
            a0_raw.ty(),
...
fn check_option_i64_builtin(
    ...
    if name == "option_and_then" {
        ...
        let a1 = coerce_checked(
            a1_raw, &expected_fn, args[1].span,
            "option_and_then fn must be fn(i64) -> Option<i64>", diagnostics,
        );
        typed_args.push(a1.expr);
    }
    CheckedExpr::new(
...
fn check_option_f64_builtin(
    ...
    if name == "option_unwrap_or_f64" {
        ...
        let a1 = coerce_checked(
            ...
            "option_unwrap_or_f64 default", diagnostics,
        );
        typed_args.push(a1.expr);
    }
    CheckedExpr::new(
...
fn check_hash_builtin(
    ...
    if matches!(name, "str_hash_pair" | "str_hash_triple") {
        for (i, a) in args.iter().enumerate() {
            let raw = check_expr(a, env, signatures, diagnostics);
            let coerced = coerce_checked(
                raw,
...
fn check_str_builtin(
    ...
    if name == "str_byte_at" {
        ...
        let s = coerce_checked(
            s_raw, &Type::Str, args[0].span,
            "str_byte_at `s` argument", diagnostics,
        );
        let i_raw = check_expr(&args[1], env, signatures, diagnostics);
        let i = coerce_checked(
            i_raw, &Type::I64, args[1].span,
            "str_byte_at `i` argument", diagnostics,
        );
        return CheckedExpr::new(
            TypedExprKind::Call {
                name: name.to_string(),
    ...
    if name == "str_len_bytes" {
        ...
        let s = coerce_checked(
            s_raw, &Type::Str, args[0].span,
            "str_len_bytes argument", diagnostics,
        );
        return CheckedExpr::new(
            TypedExprKind::Call {
                name: name.to_string(),
    ...
    if name == "str_chars" {
        ...
        let s = coerce_checked(
            s_raw, &Type::Str, args[0].span,
            "str_chars argument", diagnostics,
        );
        return CheckedExpr::new(
            TypedExprKind::Call {
                name: name.to_string(),
    ...
    if name == "str_reverse" {
        ...
        let s = coerce_checked(
            s_raw, &Type::Str, args[0].span,
            "str_reverse argument", diagnostics,
        );
        return CheckedExpr::new(
            TypedExprKind::Call {
                name: name.to_string(),
