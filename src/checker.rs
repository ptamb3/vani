...
fn check_math_builtin(
    ...
    let want_args = match name {
        ...
        | "f64_chebyshev"
        // Closure #447: i64_perm(n, k) is 2-arg.
        | "i64_perm"
    ...
    // Closure #409 + #425: i64-arg, bool-return primitives.
    if matches!(name, "i64_is_power_of_2" | "i64_is_prime") {
        let arg = check_expr(&args[0], env, signatures, diagnostics);
        let coerced = coerce_checked(
            arg, &Type::I64, args[0].span,
            &format!("{} argument", name), diagnostics,
...
fn check_str_builtin(
    ...
    // Closures #454..#457: ASCII-class counters — (Str) -> i64.
    if matches!(
        name,
