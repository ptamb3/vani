...
fn check_math_builtin(
    ...
    let want_args = match name {
        ...
        | "f64_copysign" | "f64_remainder" => 2,
        // Closure #406: f64_lerp takes 3 args. Closure #411:
        // clamp(x, lo, hi) also 3-arg. Closure #416: fma is 3-arg.
        "f64_lerp" | "i64_clamp" | "f64_clamp" | "f64_fma" => 3,
        "f64_pi" | "f64_e" | "f64_inf" | "f64_nan"
    ...
    // Closure #409: i64_is_power_of_2(n) -> bool. Special-case
    // because it's the only bit-style primitive returning bool.
    if name == "i64_is_power_of_2" {
        let arg = check_expr(&args[0], env, signatures, diagnostics);
        let coerced = coerce_checked(
            arg, &Type::I64, args[0].span,
            "i64_is_power_of_2 argument", diagnostics,
        );
        return CheckedExpr::new(
            TypedExprKind::Call {
                name: "i64_is_power_of_2".to_string(),
                name_span: span,
                args: vec![coerced.expr],
            },
            Type::Bool,
